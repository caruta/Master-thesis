...
%The \emph{Standard Model} (SM) of particle physics is the theory that we use to describe the fundamental blocks of the matter and the forces that govern their interaction. This theory includes three of the four know fundamental forces (the electromagnetic, weak and strong interactions), leaving aside the gravitational force, which is also the weakest one. (For this reason a model without it works anyway!)\\
...
%However, there are still some phenomena unexplained, the gravity is not included … so it falls short of being a complete theory of fundamental interactions.
%For these reasons some extensions of this theory have been developed in the last years and their predictions are under test in order to find a clue/hint to understand in which direction the “new physics” has to be searched for. ???
%
...
%In SM there is also another bosons called “Higgs” boson which is not a force mediator but arises from a spontaneous symmetry breaking phenomenon, the mechanism responsible for particles masses. 
...
%(nominale tutte …)
%Actually the model distinguishes 24 (not 12) different fermions because each of the ones previously cited has a corresponding antiparticle, characterized by the same mass and spin values but opposite value for the other quantum numbers.
%\end{itemize}
%
%The fundamental symmetry  group that is gauged in the standard model is 
%SU(3)$_{C} \otimes$ SU(2)$ _{L} \otimes$ U(1)$_{Y}$ , where :
%\begin{itemize}
%	\item SU(3)$_{C}$ is the \emph{color group}, the one that describes the invariance of the strong interactions when a “color transformation” is carried out. It’s the symmetry group of the Quantum Cromo Dynamics QCD
%	\item SU(2)$ _{L}\otimes$ U(1)$_{Y}$, where SU(2)$ _{L}$ is the \emph{weak isospin group} and U(1)$_{Y}$ is the \emph{hypercharge …}
...
%U(1)$_{Y}$ is different from U(1)$_{em}$, the symmetry group at the basis of the Quantum Electro Dynamics (QED) that describes the electromagnetic interactions.
%\end{itemize}
%
...
%	\item the SM has a large number of free parameters (25 includendo neurtino oscillations, otherwise 19), such as masses of charged fermions, 3 mixing angles and 1 phase of the Cabibbo-Kobayashi-Maskawa (CKM) matrix, the coupling constants of the three forces, and the mass and vacuum expectation value of the Higgs boson. 
