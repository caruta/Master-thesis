...
\begin{equation}
V(\phi^{\dagger}\phi) = k\ (\phi^{\dagger}\phi) + \lambda\ (\phi^{\dagger}\phi)^{2} = \frac{k}{2} \rho^{2} + \frac{\lambda}{4} \rho^{4}, \quad \mathrm{with}\ k<0\ \mathrm{and}\ \lambda>0\ \bigg(\frac{\rho}{\sqrt{2}} = \sqrt{\phi^{\dagger}\phi}\bigg) .
\label{eq:Higgs_pot}
\end{equation}
...
To summarize, the Standard Model Lagrangian is:
\begin{dmath}
...
In order to overcome the SM shortcomings, in the last decades several theories have been developed with the aim to extend it.

In the first part of this section I will describe the mechanism added to the SM to explain the phenomenon of neutrino oscillations and then, in the second part, I will give an overview of one of the extensions of the SM, the Minimal Supersymmetric Standard Model, in order to provide the minima theoretical tools to understand the motivations and the strategy of the physics analysis described in this thesis.

\subsubsection*{Neutrino oscillations} 
\label{Neutrino_oscillations}
The \emph{neutrino oscillation} is the phenomenon whereby a neutrino created with a specific lepton flavor ($\nu_{e}$, $\nu_{\mu}$, $\nu_{\tau}$) is later be measured to have a different flavor. It was predicted by Pontecorvo in 1957 and then included in a theory developed by Maki, Nakagawa, and Sakata 1962 \cite{ref17}.
...
If supersymmetry exists, then it is a spontaneously broken symmetry, allowing superpartners to differ in mass, since none supersymmetric particle has been observed up tp now.
...
In addition to the hierarchy problem, this theory overcomes the issues related to the gauge coupling unification (as already shown in Fig. \ref{fig:RunningCoupling} (right)) and provides good candidates for the darl matter.\\
...
Beside some fundamental quantities (energy, momentum, etc.) having a corresponding symmetry, some other conserved quantities have been introduced in the SM without having an obvious corresponding symmetry. This was done in order to account for the absence of some reactions which are in principle allowed in the theory. \\
...
In this section I will describe in detail the mechanism that allows this decay in the SM with neutrino oscillations and other eventual possibilities predicted by BSM theories, along with the expected branching fractio values from the different theories.
...
\begin{enumerate}
	\item contrarily to the radiative case $\tau^{\pm} \rightarrow \mu^{\pm} \gamma$, which is damped by a vanishingly quadratic power, leading to branching ratios(BR) of $\mathcal{O}(10^{-40})$, these kinds of decays are only suppressed by a smooth logarithmic term, therefore they are characterized by larger BR. 
...
There are different theories beyond the Standard Model which give rise to lepton flavor violation in the reach of current and future experiments. The BR of these kind of decays is indeed enhanced of several orders of magnitude.

The $\tau$ neutrinoless decays are, also in this case, the best candidate channel to study because mass-dependent couplings prefer the occurrence of lepton flavour violation in the third generation with respect to lighter leptons. \\
The decay channel $\tau \rightarrow l \gamma$ is the one with the best sensitivity if the decay $\tau \rightarrow 3l$ is mediated by a photon exchange ($\tau l \gamma$ vertex), since a diagram with a further $\gamma l l$ vertex is suppressed by a factor of $\alpha$.
However, if the mediation is done by a new heavy particle, the $\tau \rightarrow 3l$ decay channel can be favoured. 

The decay $\tau \rightarrow \mu \gamma$ is difficult to observe at the LHC, due to the backgrounds coming from $\pi_{0}$ decays and the more stringent trigger thresholds for single muons. For this reason a search for $\tau \rightarrow 3\mu$ decays had been preferred to it \cite{ref14}.

\subsubsection*{MSSM with See-Saw Mechanism}
In the SM, neutrinos are given mass through the Yukawa coupling of the fermions to the Higgs boson. The strength of these couplings are free parameters of the theory, but there is no explanation for the large difference between the couplings to neutrinos and to charged leptons. The \emph{see-saw} mechanism gives an explanation for the small neutrino masses by introducing heavy right-handed sterile neutrinos, which are assumed to be Majorana particles. The mass scale of these new particles is of the order of $10^{14}$ GeV. 

Neutrino masses can be accommodated in the MSSM by adding right-handed neutrinos with very large Majorana masses. If the right-handed neutrinos are very heavy, they induce a very small mass for the left-handed neutrinos, which is proportional to the reciprocal of their mass. The see–saw MSSM allows for a much stronger influence of the mixing in the neutrino sector on the charged lepton sector \cite{ref16}.
\newpage
The BR of the $\tau\rightarrow 3\mu$ decay can be strongly enhanced, up to $\mathcal{O}(10^{-9})$ due the mediation of neutral Higgs bosons in this process \cite{ref16b}.
...
The MSSM without R–parity includes an additional set of Yukawa interactions, between the charged leptons and sneutrinos, which are not necessarily diagonalized when the Higgs–lepton Yukawa interactions are diagonalized \cite{ref15}. 
This allows the possibility of charged lepton flavour violation mediated by a scalar particle at tree–level.
...
Concepts and Phenomena"}, Springer-Verlag, Berlin Heidelberg 1990.
	...
	\bibitem{ref14} Manuel Giffels, \emph{“Study of the Sensitivity of CMS to the Lepton Flavour Violating Neutrinoless $\tau$ Decay $\tau \rightarrow \mu \mu \mu$}, CERN-THESIS-2009-270, \url{https://cds.cern.ch/record/1292236}.
	...
	% MSSM with see-saw
	\bibitem{ref16b} K.S. Babu and C. Kolda, \emph{“Higgs-Mediated $\tau \rightarrow \mu \mu \mu$ in the Supersymmet- ric Seesaw Model"}, Phys. Rev. Lett., 89 (2002), 241802, \href{https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.89.241802}{doi:10.1103/PhysRevLett.89.241802}, \href{https://arxiv.org/abs/hep-ph/0206310}{arXiv:hep-ph/0206310v2}.
