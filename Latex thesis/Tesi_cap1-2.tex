...
The Higgs boson is a CP-even scalar of spin 0. Today, the measured value for the Higgs boson mass is \cite{ref112}:
\begin{equation}
...
\end{figure}
The dominant mechanism for Higgs boson production and decay involve the coupling of the Higgs boson to W, Z and the third generation of quarks and leptons, which are the heaviest particles.
...
The main Higgs decay modes are shown in Fig. \ref{fig:Higgs_decay}. The dominant ones are: $H \rightarrow b\bar{b}$, $H \rightarrow WW^{*}$, followed by $H \rightarrow \gamma\gamma$, $H \rightarrow \tau^{+}\tau^{-}$ and $H \rightarrow ZZ^{*}$.
...
\end{figure}
\newpage
The last important results obtained by CMS and ATLAS experiments in the physics of the Higgs boson are \cite{ref112}:
\begin{itemize}
...
In order to overcome the SM shortcomings, in the last decades several theories aiming to extend the SM have been developed.\\
In the first part of this section I will describe the mechanism added to the SM in order to explain the phenomenon of neutrino oscillations and then, in the seconf part, I will give an overview of one of the BSM theories, the Supersymmetry, in order to provide the minima theoretical tools to understand the motivations and the strategy of the analysis described in this thesis, which will be extensively treated in (REF).

\subsubsection*{Neutrino oscillations} 
\label{Neutrino_oscillations}
The \emph{neutrino oscillation} is the phenomenon whereby a neutrino created with a specific lepton flavor ($\nu_{e}$, $\nu_{\mu}$, $\nu_{\tau}$) is later be measured to have a different flavor. It was predicted by Bruno Pontecorvo in 1957 and then explained in a theory developed by Maki, Nakagawa, and Sakata, with the contribution of Pontecorvo, in 1967 \cite{ref17}.\\
The first experimental observation of this phenomenon was made by the Super-Kamiokande and the Sudbury Neutrino Observatories \cite{ref17a}, which receveid for this reason a Nobel Prize for Physics in 2015.

The neutrino oscillation is due to the mixing between the flavor eigenstates (the one in which neutrinos are when they are emitted and absorbed in weak processes) and mass eigenstates (the ones in which neutrinos propagates through space). 
Therefore, when neutrinos travel, the phases of their mass states change at slightly different rates, due to the small differences in their respective masses,resulting in a changing superposition mixture of mass eigenstates.
...
In its simplest form this phenomenon can be expressed as a unitary transformation relating the flavor and mass eigenbasis and can be written as:
\begin{equation}
\ket{\nu_{\alpha}} = \sum_{i}U^{*}_{\alpha i}\ket{\nu_{i}} \qquad \ket{\nu_{i}} = \sum_{\alpha}U_{\alpha i}\ket{\nu_{\alpha}}
\end{equation}
where: $\ket{\nu_{\alpha}}$  is a neutrino with definite flavor $\alpha$( = e, $\mu$ or $\tau$) and $\ket{\nu_{i}}$ is a neutrino with definite mass $m_{i}, i=1,2,3$.

$U_{\alpha i}$ is the \emph{Pontecorvo–Maki–Nakagawa–Sakata matrix} (PMNS matrix), which is the analogue of the CKM matrix describing the mixing of quarks. It is a  3$\times$3 matrix given by:
\begin{equation}
U_{\alpha i} =
\begin{pmatrix}
...
The \emph{Minimal Supersymmetric Standard Model} is an extension of the SM that uses a new fermion-boson symmetry, called \emph{supersymmetry}.
To each particle from one group (fermion or boson) is associated a particle of the other group (emph{superpartner}). Therefore the spin of these  two particles differs by a half-integer.\\
If supersymmetry exists then it is a spontaneously broken symmetry allowing superpartners to differ in mass, since no one of them as never been observed up tp now.

The MSSM was originally proposed in 1981 to solve the hierarchy problem. It still predictes an elementary Higgs boson but the corrections to its mass are screened at the scale at which the supersymmetry is broken and remain insensitive to the details of the physics at higher scales. Moreover, it predict also other neutral Higgs particles and a pair of charged Higgs. One of the neutral Higgs bosons, the lightest one, has properties that resemble those of the SM Higgs boson \ref{112}.

In addition to the hierarchy problem, this theory overcomes also the issues related to the gauge coupling unification (as already shown in Fig. \ref{fig:RunningCoupling} (right) and provides good candidates for the darl matter.\\
This is a consequence of the introduction of the \emph{R-parity}, which was firstly required in order to explain the stability of the proton.
...
As already said, this parity guarantees the stability of the protons but it also ensures that the lightest supersymmetric particle (LSP) is stable and is a \emph{Weakly interacting Massive Particle} (WIMP). This means that it barely does not interact electromagnetically or strongly, which makes the LSP a good dark matter candidate.

Several additional theories with other requirements have been developed starting from the MSSM, and two of them (Supersymmetric Model with R-Parity Violation and the Supersymmetric Model with See-Saw Mechanism) will be presented in more detail in (REF) because are the ones whose predictions are under test in the physics analysis that I have carried out.

\section{Lepton Flavor Violation (LFV)}
The heart of this thesis work is the search for a Lepton flavor violation in the decay: $\tau \rightarrow 3 \mu$. In this section will be described in detail the mechanism through which this decay can happen and the expected branchiing fractio values both in the SM and in some BSM theories.

There exist an important theorem which provides a connection between the symmetry of a system and its conserved quantities: the \emph{Noether's theorem} \cite{ref14a}.
It states that every symmetry of the action (which is the integral over time of a Lagrangian) of a physical system has a corresponding conservation law. \\ For example, if a physical system is invariant under translations in space and time, the momentum and the energy Are conserved quantities, if the system is invariant under rotations, the angular momentum is conserved.\\
It is a very powerful tool because it allows to determine the conserved quantities from the observed symmetries of a physical system.

Beside these fundamental quantities (energy, momentum, charge, etc.) having a corresponding symmetry, some conserved quantities have been introduced to the Standard Model without having an obvious corresponding symmetry. This was done in order to account for the absence of some reactions which are in principle allowed in the Standard Model. \\
This is the case of the lepton flavor.

In 1953 Konopinski and Mahmoud proposed the law of conservation of lepton number, in order to introduce a simple rule to determine which reactions are allowed and which are not \cite{ref14b}. The lepton number is defined as : $L$ = 1 for all the leptons and $L$ = -1 for all their antiparticles (??).\\
However, this conservation law was not enough to explain the absence of observation of decays like $\tau \rightarrow \mu \gamma$.

Therefore, a more stringent version of this law was proposed, in which was reuired the conservation of the lepton flavor. The lepton flavor is the same for the leptons belonging to the same family ($e^{-}$ and $\nu_{e}$, $\mu^{-}$ and $\nu_{\mu}$, $\tau^{-}$ and $\nu_{\tau}$ and has an opposite value for the corresponding antiparticles.


\subsection{LFV in the Standard Model with neutrino oscillations}
Today, it is known that the conservation law of the lepton flavor is not strict, due to the existence of neutrino oscillations. 
In the extended version of the SM including neutrino oscillations, the LFV decay $\tau \rightarrow 3 \mu$ is allowed and the main contribution to it comes from the process described in the Feynman diagram in Fig. \ref{fig:FD_SM}.
...
\subsection{LFV in BSM theories - G}
There are different models beyond the Standard Model which give rise to lepton flavor violation in the reach of current and future experiments. Mass-dependent couplings prefer the occurrence of lepton flavour violation in the third generation with respect to lighter leptons. 

(BSM theories allow these LFV decays with a BR which is enhanced of several orders of magnitude, resulting in cross section values which can be tested with current experiments[ref. AN].)

The decay channel $\tau \rightarrow l \gamma$ has in general a better sensitivity, if the decay $\tau \rightarrow lll$ is mediated by a photon exchange ($\tau l \gamma$ vertex), since a diagram with a further $\gamma l l$ vertex is suppressed by a factor of $\alpha$ 
However, if the mediation is done by a new heavy particle, the $\tau \rightarrow lll$ decay channel can be favoured (8). 
The decay $\tau \rightarrow \mu \gamma$ is difficult to observe at the LHC, due to the backgrounds coming from $\pi_{0}$ decays and the more stringent trigger thresholds for single muons. 
\subsubsection*{Supersymmetric Model with See-Saw Mechanism}
The Minimal Supersymmetric Standard Model (MSSM) is a possible extension of the Standard Model. The Supersymmetry (SUSY) is a symmetry between bosons and fermions. The MSSM contains beside the well-known Standard Model particles also their supersymmetric partners [10]. A detailed introduction into Supersymmetry can be found in [11]. 

In the Standard Model, neutrinos are given mass through the Yukawa coupling of the fermions to the Higgs boson. In principle the strength of these couplings are free pa- rameters of the Standard Model, but there is no explanation for the large difference between the couplings to neutrinos and to charged leptons. The see-saw mechanism, which gives an explanation for the tiny neutrino masses, introduces heavy right-handed sterile neutrinos, which are assumed to be Majorana particles. The mass scale of these new particles is of the order of $10^{14}$ GeV. The see-saw mechanism can be also used in the MSSM to generate the neutrino masses [8, 10]. 
Depended on the model parameters, lepton flavour violation can occur in the couplings of the neutral Higgs bosons. This effect is strongly enhanced for large tan $\beta$. The con- tributing Feynman diagrams are depicted in figure 
...
A prediction of the branching 
Outline Introduction New Physics Models with $\tau \rightarrow \mu \mu \mu$ Backup Slides
ratio of the $\tau \rightarrow \mu \mu \mu$ decay for specific assumptions on the model parameters is  (vedi pag. 13)
(A detailed description of the calculation and the assumed model parameters can be found in [12].)
...
-MOTIVI PER LA SCELTA DEL CANALE DELL'ANALISI	
