...
The Standard Model classifies the particles according to their intrinsic angular momentum, the spin, dividing them in two main categories:	
\begin{itemize}
...
In the SM there is also another boson, called \emph{Higgs} boson, which is not a force mediator but arises from a spontaneous symmetry breaking phenomenon, the mechanism responsible for particles masses. 
...
Fermions are divided in 2 groups: \emph{leptons} (which have an integer electric charge) and \emph{quarks} (characterized by a fractional electric charge).\\ Each of these group is made up of 6 particles, paired in 3 generations, as shown in the Table/Figure.
(nominale tutte …)
Actually the model distinguishes not only 12, but 24 different fermions, since each of the ones previously cited has a corresponding antiparticle, characterized by the same mass and spin values but opposite value for the other quantum numbers.
\end{itemize}

The fundamental symmetry  group that is gauged in the Standard Model is 
SU(3)$_{C}\ \otimes$ SU(2)$ _{L}\ \otimes$ U(1)$_{Y}$ , where:
\begin{itemize}
...
The Quantum Chromodynamics describes the interaction between quarks and gluons.
This theory is based on the symmetry group SU(3)$_{C}$,

 because the multiplets (quali?) are invariant under “color transformations”.
 
 The symmetry group SU(3)$_{C}$ is non-Abelian (this is the reason why the QCD phenomenology is completely different from that of QED, which is described by the group U(1)$_{em}$, which is clearly Abelian).
In fact this has as a consequence the fact that gluons are "self-interactive".

The \emph{color} is a quantum number which is describes the fact that quarks can be have 3 different values for this value. However, in nature only colorless states can be observed, and this is explained by the fact that they are singlets under rotations in colour space \emph{confinement hypothesis}. A consequence of this is that quarks can never be observed as "free" (because they carry colour) and they are always confined within colour-singlet bound states.


The QCD Lagrangian is given by the equation \ref{eq:QCD_Lagr}.

\begin{equation}
\mathcal{L}_{QCD} = -\frac{1}{4} G_{\mu\nu}^{a}(x) + \sum_{j=1}^{f} [i \gamma^{mu}D_{\mu - m_{j}]q_{j}(x)}
\label{eq:QCD_Lagr}
\end{equation}
where:
\begin{itemize}
	\item the covariant derivative $D_{\mu}$ is defined as: $\partial_{\mu} + ig_{S}G_{\mu}^{a}(x)\frac{\lambda_{a}}{2}$
	\item The \emph{gluon field strength tensor} $G_{\mu\nu}$ is defined as: $G_{\mu\nu}(x) = \partial_{\mu}G_{\nu}(x)-\partial_{\nu}G_{\mu}(x) + ig_{S}[G_{\mu\nu}(x), G^{\mu\nu}(x)]$ and its components are defined by : $G_{\mu\nu}(x) = G_{\mu\nu}(x)^{a}\frac{\lambda_{a}}{2}$
\end{itemize}

(It can be shown that this Lagrangian is invariant under arbitrary global SU(3)$_{C}$ transformations in colour space)

\subsubsection*{Electroweak theory}
The electroweak theory is the product of the unification of the QED and the weak theory, carried out by Glashow in 1961.
...
\begin{equation}
\mathcal{L}_{SM} = -\frac{1}{4} G_{\mu\nu}G^{\mu\nu} -\frac{1}{2} W_{\mu\nu}W^{\mu\nu} -\frac{1}{4} B_{\mu\nu}B^{\mu\nu} + \bar{\psi} i \gamma^{\lambda}D_{\lambda}\psi + (D_{\lambda}\phi)^{\dagger} (D^{\lambda}\phi) - V(\phi) + \mathcal{L}_{Yukawa}
\label{eq:SM_Lagr}
\end{equation}
where:
\begin{itemize}
