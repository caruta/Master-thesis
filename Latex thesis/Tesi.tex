...
%The \emph{Standard Model} (SM) of particle physics is the theory that we use to describe the fundamental blocks of matter and the forces that govern their interactions. This theory includes three of the four known fundamental forces (electromagnetic, weak and strong interactions), leaving aside the gravitational force, which is the weakest one. This is the reason why this model provides extremely precise predictions even if it is not complete.\\
...
%For each of this 12 different fermions there is a corresponding antiparticle, characterized by the same mass and spin, but opposite value for the other quantum numbers. 
%%Therefore, in total, the model distinguishes 24 fermions.
%\end{itemize}
%
%\begin{figure}[h]
%	\centering
...
%Moreover, the minimal coupling between the quarks and the gluons can be introduced by replacing the derivative with a covariant derivative $D_{\mu}$, defined as: $D_{\mu} = \partial_{\mu} + ig_{s}G_{\mu}$, being $g_{s}$ the dimensionless coupling constant of the strong forces. \\
...
%In this representation the symmetry of the field is expressed explicitly, and it can be represented by Fig. \ref{fig:V_Higgs2D}. \\
...
%After having treated in detail the two fundamental theories at the bases of the SM (QCD and electroweak theory) and having introduced the mechanism that gives mass to the particles, it is possible to use all of these tools to write the Standard Model Lagrangian:
%\begin{dmath}
...
%\end{itemize}
%
%\subsection{Status of the Higgs boson physics and last observations}
...
%The mechanism with the largest cross section is the \emph{gluon fusion}: $gg \rightarrow H + X$, shown in \ref{fig:Higgs_prod}a. It is mediated by the exchange of a virtual top
...
%\begin{itemize}
%	\item observation of the Higgs boson decay to taus, made by the CMS \cite{ref10c},
...
%	\item measurement of properties of $B^{0}_{s}\rightarrow \mu^{+} \mu^{-}$ decay, which is an ultra rare decay of a beauty and a strange meson, carried out by CMS \cite{ref10f}.
%\end{itemize}
%
...
%
%In the first part of this section I will describe the mechanism added to the SM to explain the phenomenon of neutrino oscillations and then, in the second part, I will give an overview of one of the extensions of the SM, the Minimal Supersymmetric Standard Model, in order to provide the minima theoretical tools necessary to understand the motivations and the strategy of the physics analysis described in this thesis.
%
%\subsubsection*{Neutrino oscillations} 
%\label{Neutrino_oscillations}
%The \emph{neutrino oscillation} is the phenomenon whereby a neutrino created with a specific lepton flavor ($\nu_{e}$, $\nu_{\mu}$, $\nu_{\tau}$) is later be measured to have a different flavor. It was predicted by Pontecorvo in 1957 and then included in a theory developed by Maki, Nakagawa, and Sakata in 1962 \cite{ref17}.
...
%It is a  3$\times$3 matrix, which can be parametrized by 3 angles ($\theta_{ij}$) and, depending on whether the neutrinos are Dirac or Majorana particles (a Majorana neutrino is identical to its antineutrino), by 1 ($\delta$) or 3 ($\delta, \alpha_{1}, \alpha_{2}$) CP violation phases:
%
%\begin{dmath}
%U_{\alpha i} =
%\begin{pmatrix}
...
%To each particle from one type (fermion/boson) this symmetry associates a particle of the other type (boson/fermion), called \emph{superpartner}. Therefore the spin of the two particles differs by a half-integer.\\
%If supersymmetry exists, then it is a spontaneously broken symmetry, allowing superpartners to differ in mass, since none supersymmetric particle has been observed up to now.
%
%The MSSM was originally proposed in 1981 to solve the hierarchy problem. It still predictes an elementary Higgs boson but the corrections to its mass are screened at the scale at which the supersymmetry is broken and remain insensitive to the details of the physics at higher scales. Moreover, this model predicts also other neutral Higgs particles and a pair of charged Higgs. One of the neutral Higgs bosons, the lightest one, has properties that resemble those of the SM Higgs boson \cite{ref112}.
%
...
%\end{cases}
%\end{equation}
%where $R=3B + L + 2S$ with $B$ and $L$ being respectively the baryon and the lepton numbers, and $S$ the spin of the particle.\\
%This parity guarantees the stability of the proton but it also ensures that the lightest supersymmetric particle (LSP) is stable and is a \emph{Weakly interacting Massive Particle} (WIMP). This means that it does not interact electromagnetically or strongly, which makes the LSP a good dark matter candidate.
%
...
%In physics there exist a fundamental theorem which provides a connection between the symmetry of a system and its conserved quantities: the \emph{Noether's theorem} \cite{ref14a}.
%It states that every symmetry of the action (which is the integral over time of the Lagrangian) of a physical system has a corresponding conservation law. For example, if a physical system is invariant under translations in space and time, the momentum and the energy are conserved quantities and if it is invariant under rotations, the angular momentum does not change.\\
...
%
%Beside some fundamental quantities (energy, momentum, etc.), having a corresponding symmetry, some other conserved quantities have been introduced in the SM without having a known symmetry. This was done in order to account for the absence of some reactions which are in principle allowed in the theory. \\
...
%However, this conservation law was not enough to explain the absence of observation of decays like $\tau \rightarrow \mu \gamma$. Therefore, a more stringent version of the law was proposed, in which it was required the conservation of the lepton flavor. This quantity is identical for leptons belonging to the same family ($e^{-}$ and $\nu_{e}$, $\mu^{-}$ and $\nu_{\mu}$, $\tau^{-}$ and $\nu_{\tau}$) and has an opposite value for their antiparticles.
%
%The search for a charged lepton flavor violation is the heart of the work of this thesis. In particular, this violation will be searched in the decay channel $\tau \rightarrow 3 \mu$. 
%
%In this section I will describe in detail the mechanism that allows this decay in the SM with neutrino oscillations and other eventual possibilities predicted by BSM theories, along with the expected branching fractio values from the different models.
%
...
%In the extended version of the SM including neutrino oscillations, LFV decays are allowed and the study of them can enveil a wealth of information regarding the SM limits and the possible extensions of it.\\
...
%There are different theories beyond the Standard Model which give rise to lepton flavor violation in the reach of current and future experiments. The BR of these kinds of decays is indeed enhanced of several orders of magnitude.
%
%The $\tau$ neutrinoless decays are, also in this case, the best candidate channels to be studied because mass-dependent couplings prefer the occurrence of lepton flavor violation in the third generation with respect to lighter leptons. \\
...
%The MSSM with R–parity violation includes an additional set of Yukawa interactions, between the charged leptons and sneutrinos, which are not necessarily diagonalized when the Higgs–lepton Yukawa interactions are diagonalized \cite{ref15}. 
...
The chosen of the decay $\tau \rightarrow 3 \mu$ is motivated by several reasons:
\begin{itemize}
	\item As already seen in (REF paragrafo), Other kind of LFV decays are expected to be extremely suppressed, with a BR many orders of magnitude smaller! (For example: $\mathcal{B}(\tau \rightarrow \mu\gamma\ \sim \mathcal{O}(10^{- 40})$). Moreover the enhancement of the BR is mass-dependent!
...
\begin{table}[h!]\footnotesize
	...
	\begin{tabular}{|l|l|c|}
	\hline 
	Process1		&	Process2	& 	N. of $\tau$ for L = ? \\ \hline \hline
...
\subsection{Outline: search strategy}
...
\begin{table}[h!]\footnotesize
	...
	$D_{s} \rightarrow \phi \pi \rightarrow 2 \mu \pi$	&	/DsPhiPi/fsimone-crab\_crab_DsPhiPi\_13TeV\_RECO-	&	\\
		& c3763c515b5d7d94a8137c090655e1bb/USER\ PRIVATE	&	000\\
	\hline
	\end{tabular}
...
\begin{enumerate}
	...
	\item At least 3 muons with :
		...
		\end{itemize}
	\item At least 1 triplet with charge = $pm$ 1
\end{enumerate}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{NDs_total}
	\caption{Full 2017 data in $D_{s} \rightarrow \phi(\mu\mu)\pi$ channel. The $D_{s}$ and $D^{+}$ peaks are fitted with Crystal Ball functions, while the background is fitted with an exponential.}
	\label{fig:NDs_total}
\end{figure}
...
\begin{table}[h!]\footnotesize
	...
	\begin{tabular}{|c|c|c|}
	\hline
	Run 	& 	Fitted $D_{s}\rightarrow\phi(\mu\mu)\pi$ yields ($fb^{-1}$)	&	Data/MC ratio\\ \hline 
	2017 B	&	000	&	000\\
	2017 C	&	000	&	000\\
	2017 D	&	000	&	000\\
	2017 E	&	000	&	000\\
...
\begin{figure}[h!]
 \begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{NDs_runB}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{NDs_runC}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{NDs_runD}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{NDs_runE}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=7.2cm]{NDs_runF}
...
\newpage
Spiega calcolo sistematics sul numero Ds bla bla... shown in Fig. \ref{fig:Systematics_Ds}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm]{Systematics_Ds}
	 \caption{For each of 2017 runs, the $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields and the $D_{s} \rightarrow 3\mu$ yields per $fb^{-1}$ are reported on the left, while their ratio is displayed on the right. (Eventuali richieste trigger ? Es. DoubleMu L1 seeds)}
...
\begin{enumerate}
	\item Fired HLT path: \emph{HLT\_DoubleMu3\_Trk\_Tau3mu\_v*}
...
\subsubsection{Training, variable ranking, performance }
...
Concepts and Phenomena"}, Springer-Verlag, Berlin Heidelberg 1990.
	...
	% Last discoveries related to Higgs
