...
\end{equation}
being $g_{s}$ the dimensionless coupling constant of the strong forces. \\
...
\end{equation}
Actually $\alpha_{s}$ this is not a constant, but its value depends on the energy: it is larger at lower energies and it decreases with the increase of the energy (or with the decreasing of the distance between the interacting particles), thus explaining the confinement of quarks.
...
\\
The $B^{0}_{s}\rightarrow \mu^{+} \mu^{-}$ decay is an ultra rare decay of a beauty and a strange meson, which is strongly suppressed in the SM due to the GIM mechanism. For this reason it cannot occur at tree level with the exchange of $W$ bosons and the expected BR is of the order of $10^{-9}$. The corresponding value measured by CMS is $2.9 \cdot 10^{-9}$, which is consistent with the value measured by LHCb experiment and with the SM prediction. Moreover, no significant excess was observed for the decay $B_{0} \rightarrow \mu^{+}\mu^{-}$, leading to the determination of an upper limit $\mathcal{B}(B_{0} \rightarrow \mu^{+}\mu^{-}) < 3.6 \cdot 10^{-10}$ at 95\% CL.
...
As discussed in section \ref{symm}, many fundamental conservation laws are the consequence of the Noether's theorem. Additional conservation laws have been introduced in the SM without an underlying simmetry, in order to account for the lack of observation of some processes which are in principle allowed in the theory. 

Togli (Beside some fundamental quantities (energy, momentum, etc.), having a corresponding symmetry, some other conserved quantities have been introduced in the SM without having a known symmetry, in order to account for the lack of observation of some processes which are in principle allowed in the theory.) \\
