...
In this chapter the search for the decay $\tau \rightarrow 3\mu$ using the data collected by CMS in pp collisions during the 2017 data-taking at a center of mass energy of 13 TeV, will be described. 
First of all some preliminary information about previous results obtained by other experiments searching for this decay will be given, along with technical information ??.\\
Then the search strategy will be outlined, and each step of the analysis will be described in details and finally, the obtained results will be discussed.
...
The expected inclusive number of $\tau$ coming from different processes, for the integrated luminosity of the 2017 data-taking, i.e. $\mathcal{L} = 38\ fb^{-1}$, is reported in Tab. \ref{tab:ExpectedTau}. 
...
\begin{table}[h!]\footnotesize
	...
	\hline
	\end{tabular}
	\caption{List of processes from which $\tau$ leptons are produced, with their corresponding expected inclusive number for $\mathcal{L} = 38\ fb^{-1}$. The charge-conjugated states are included. }
	\label{tab:ExpectedTau}
\end{table}

Non so se ho già detto da qualche parte che noi abbiamo scelto di guardare in particolare ai tau prodotti dai mesoni con heavy flavor, ma che in realtà un certo numero di tau è anche prodotto dai bosoni W e Z (vedi Manzoni).

The relative $\tau$ yields are listed in Tab. \ref{tab:relativeTauYield}. It is possible to notice that the largest contribution to the number of $\tau$ comes from the decay of $D$ mesons, which happens in 75\% of the cases (in particular from the decay of the $\mathrm{D_{s}}$ mesons), but there is also a contribution from the decay of the $B$ mesons, which occurs in the 25\% of the total cases.
...
(from raffa: The data recorded by CMS are stored in several different datasets. All events that are accepted by a specific set of high level triggers enter one specific dataset, so that the choice of a trigger for the analysis defines which dataset has to be used.
During 2017 there have been 5 running periods labeled from B to F. 
The datasets used in this analysis are the \emph{DoubleMuonLowMass} datasets, which are listed in Tab. \ref{tab:2017datasets}.\\
They correspond to a total integrated luminosity: $\mathcal{L} = 38\ fb^{-1}$.\\
...
(Only runs that satisfy the data quality criteria were used in the analysis. These runs are listed in a file in the JSON format)
...
The integrated luminosity of the whole 2017 is of $\sim38 fb^{-1}$. 
...
from Raffa translated - In MC simulations is important to consider the pile up effect, that is present in each event in CMS (as already explained in -REF).
...
The "function of the weights" that must be multiplied to the MC events (? spiega bene) in order to "reweighting" it, id est the value of the weight computed with the eq. \ref{eq:PU_rew} as a function of number of pile-up events, is reported in Fig. \ref{fig:PU_weights}.
...
- Specificare bene a quale MC sto facendo riferimento (DsTau3MU).

The effect of the pile-up reweighting is visibile especially in the distribution of the reconstructed vertices, as is shown in Fig. \ref{fig:PU_Vertices}.
...
\begin{minipage}[b]{7.3cm}
 ...
 \end{minipage}
    \caption{N of primary vertices in MC before (on the left) and after (on the right) the pile-up reweighting, compared to the same quantity in data. From this comparison the importance of the pile-up reweighting procedure is clear. (rifrasa?)}
	\label{fig:PU_Vertices}
 \end{figure}

\section{Search strategy}
The events with D and B mesons decaying in a tau lepton that in turn decays in 3 muons, are characterized by low pT muons, therefore the selection of the muon candidates must be optimised for muons with these characteristics, but at the same time they should reject the $\pi$ and $K$ mesons, which can "easily" be misidentified and recostructed as low pT muons... (since CMS is not really optimized to recognize this kind of muons...)
Attenzione anche ai muons from $K$ and $\pi$ decays in flight!
...
\begin{figure}[h]
	\centering
	\includegraphics[width=12cm]{NDs_total}
...
\begin{figure}[h!]
 \begin{minipage}[b]{7.4cm}
  \centering
   \includegraphics[width=7.3cm]{Systematics_Ds}
 \end{minipage}
\begin{minipage}[b]{7.4cm}
	\centering
	\includegraphics[width=7.3cm]{Systematics_Ds_ratio}
\end{minipage}
