...
%However, it is evident that this is an effective theory, and therefore it needs to be extended in order to include the gravitation and to provide a description of some experimental observations which are still unexplained.
...
%Finally, the Lagrangian density in \ref{eq:FreeQuarks_Lagr} is invariant under gauge transformations iff the gluon fields transform as: 
%\begin{equation}
%G_{\mu}(x) \rightarrow G'_{\mu}(x) = U(x)G_{\mu}(x)U^{\dagger}(x) - \frac{i}{g_{s}}U(x)\partial_{\mu}U^{\dagger}(x)\ .
%\end{equation}
%A last term in the Lagrangian has to be included in order to account for the gluon dynamics, which is still missing. In order to do it, a \emph{gluon field strength tensor} $G_{\mu\nu}$ must be introduced. It is defined as: 
%\begin{equation}
...
%\label{eq:QCD_Lagr}
%\end{equation}
%SU(3)$_{C}$ is an exact symmetry: in \ref{eq:QCD_Lagr} there is not a term $\propto G^{\mu}G_{\mu}$ because it would be $\propto m^{2}$ and it would break the gauge invariance. Therefore gluons, like photons in QED (where also $U(1)_{em}$ is an exact symmetry), are massless, and they have a spin value equal to 1.\\
...
%
%In order to overcome this problem, Weinberg and Salam, with the contribution of Veltmann and ‘t Hooft, added to the theory the \emph{Spontaneous Symmetry Breaking} (SSB) mechanism hypothesized in 1964 independently by Higgs \cite{ref6, ref8} and by Englert and Brout \cite{ref7}. This mechanism was able to explain the mass of the particles without violating the gauge invariance of the Lagrangian. Contributions from Guralnik, Hagen and Kibble \cite{ref9} were also fundamental to understand its properties.
%
...
%
%\subsubsection*{Finally, the SM Lagrangian}
%After a detailed discussion on the two fundamental theories at the bases of the SM (QCD and electroweak theory) and the introduction of the mechanism that gives mass to the particles, it is possible to use all of these tools to write the Standard Model Lagrangian:
%\begin{dmath}
...
%
%\newpage
%\subsection{Status of SM observations at LHC}
%\label{lastObs}
%The ATLAS and CMS experiments at the Large Hadron Collider, on the 4th of July 2012 announced the observation a new particle, consistent with the Higgs boson predicted by the SM, in the mass region around 125 GeV  \cite{ref1, ref2}. \\
...
%\\
%\\
%The main Higgs decay modes are shown in Fig. \ref{fig:Higgs_decay}. The dominant ones are: $H \rightarrow b\bar{b}$, $H \rightarrow WW^{*}$, followed by $H \rightarrow \tau^{+}\tau^{-}$, $H \rightarrow c \bar{c}$, $H \rightarrow ZZ^{*}$ and $H \rightarrow \gamma\gamma$.\\
%\begin{figure}[h!]
%  \centering
...
%\\
%The $B^{0}_{s}\rightarrow \mu^{+} \mu^{-}$ decay is an ultra rare decay of a beauty and a strange meson, which is strongly suppressed in the SM due to the GIM mechanism. For this reason it cannot occur at tree level with the exchange of $W$ bosons and its expected BR is of the order of $10^{-9}$. The corresponding value measured by CMS is $2.9 \cdot 10^{-9}$, which is consistent with the value measured by LHCb experiment and with the SM prediction. Moreover, no significant excess was observed for the decay $B_{0} \rightarrow \mu^{+}\mu^{-}$, leading to the determination of an upper limit $\mathcal{B}(B_{0} \rightarrow \mu^{+}\mu^{-}) < 3.6 \cdot 10^{-10}$ at 95\% CL.
%
%
%\newpage
...
%
%
%\subsubsection*{Minimal Supersymmetric Standard Model (MSSM)}
%The \emph{Minimal Supersymmetric Standard Model} is an extension of the SM based on a new fermion-boson symmetry, called \emph{supersymmetry}.
...
%Since any supersymmetric particle has never been observed up to now, the supersymmetry should be a spontaneously broken symmetry, allowing superpartners to have a larger mass with respect to the observed SM particles.
%
...
%This is, for example, the case of the lepton flavor.
%
...
%However, this conservation law was not enough to explain the absence of observation of decays like $\tau \rightarrow \mu \gamma$. Therefore, a more stringent version of the law was proposed, in which the conservation of the lepton flavor was required. The lepton flavor is identical for leptons belonging to the same family ($e^{-}$ and $\nu_{e}$, $\mu^{-}$ and $\nu_{\mu}$, $\tau^{-}$ and $\nu_{\tau}$) and has an opposite value for their antiparticles. Thus, there exist 3 possible flavors for leptons ($L_{e}$, $L_{\mu}$, $L_{\tau}$) and they should be conserved separately.
%
%The search for a process that violates the conservation of the flavor in case of charged leptons is the heart of the work of this thesis. In particular, this violation will be searched in the decay channel $\tau \rightarrow 3 \mu$. 
%
...
%
%\subsection{LFV in the SM with neutrino oscillations}
...
%\begin{enumerate}
%	\item contrarily to the radiative case $\tau^{\pm} \rightarrow \mu^{\pm} \gamma$, which is damped by a vanishingly quadratic power, leading to branching ratio (BR) of $\mathcal{O}(10^{-40})$, these kinds of decays are only suppressed by a smooth logarithmic term, therefore they are characterized by larger BR,
...
%\end{figure}
%
%\subsection{LFV in BSM theories}
...
%
%The decay $\tau \rightarrow \mu \gamma$ is difficult to observe at the LHC, due to the backgrounds coming from $\pi_{0}$ decays and the more stringent trigger thresholds for single muons \cite{ref14}. For this reason a search for $\tau \rightarrow 3\mu$ decays had been preferred to it.
%
...
\newpage
\chapter{Analysis}
In this chapter I will be described the search for the $\tau \rightarrow 3\mu$ decay carried out using the $\tau$ leptons produced from the decay of $D$ and $B$ mesons, in data collected by CMS experiment in pp collisions during the 2017 data-taking, at a center of mass energy of 13 TeV. \\
The motivations underlying the choice of this analysis have been extensively described in section \ref{LFV}. There can be just recalled the most important of them, i.e. the fact that, form one side LHC is a tau factory and on the other side CMS is very performant in the muon detection, as well as the large enhancement in the branching fraction of this decay expected from some BSM theories.

In the first part of the chapter some information about previous searches for this decay will be provided, along with preliminary considerations about the feasibility of this search based on the expected number of tau leptons and the signal acceptance in CMS.\\
Then, in the second part, the strategy of this search will be outlined, and each step of the analysis will be described in details together with the obtained results.
...
The best experimental value set for this upper limit, up to now, is the one obtained by Belle experiment: $\mathcal{B}(\tau \rightarrow 3\mu\ \leq\ 2.1\cdot10^{-8}$ at 90\% confidence level \cite{ref102}. A similar result was obtained by BaBar, another experiment operating, as Belle, at an $e^{+}e^{-}$ collider and now decommissioned; it set the upper limit at $3.3\cdot10^{-8}$ \cite{ref103}.
...
\begin{itemize}
	...
	\item CMS (only 2016 data - 33 $fb^{-1}$) : 8.8 $\times 10^{-8}$ \cite{ref108a}.\\
	Nominare Manzoni ?
\end{itemize}
...
At LHC $\tau$ leptons can be produced in different processes, and the expected inclusive production cross section, at a center of mass energy of 13 TeV, is of $\sim$ 2 $\times$ 10$^{11}$ fb. The expected inclusive number of them, for the different processes, for an integrated luminosity of $\mathcal{L} = 38\ fb^{-1}$ (which is the one of 2017), are listed in Tab. \ref{tab:ExpectedTau}. 

\begin{table}[h!]\footnotesize
	\centering
	\begin{tabular}{|l|l|c|}
	\hline 
	Process1		&	Process2	& 	N. of $\tau$ for $\mathcal{L} = 38\ fb^{-1}$ \\ \hline \hline
	...
	\hline
	\end{tabular}
	\caption{List of processes from which $\tau$ leptons are produced at LHC, with the corresponding expected inclusive numbers for $\mathcal{L} = 38\ fb^{-1}$. The charge-conjugated states are included.\\ The expected number of $\tau$ coming from decays of $D/B$ mesons was estimated using the LO event generator PYTHIA 8  \cite{ref104} set for inelastic pp interactions using a total inelastic cross section of 80 mb. The expected number of $\tau$ produced by $W$ and $Z$ bosons was extimated using the NNLO $pp \rightarrow W \rightarrow \tau \nu_{\tau}$ and $pp \rightarrow Z \rightarrow \tau \tau$ cross sections at 13 TeV 19.7 and 1.87 nb, respectively.}
	\label{tab:ExpectedTau}
\end{table}

As it is evident from the table, the main sources of $\tau$ leptons are the hadrons containing $c$ or $b$ quarks, in particular they are essentially $D$ and $B$ mesons, and only rarely $c/b$-baryons. Actually, this last contribution is so small in comparison to direct D/B-decays therefore, it is henceforth ignored.\\ 
Another possible source of these leptons are the $W$ and $Z$ bosons, that can decay in the following ways: $W \rightarrow \tau \nu_{\tau }$ and $Z\rightarrow \tau^{+}\tau^{-}$. \\
The leptons originated from the $D$ and $B$ mesons have different characteristics from the ones produced by the decay of $W$ and $Z$ bosons (The average transverse momenta of last taus are expected to be very high in comparison to the ones coming from B/D mesons of direct production), therefore different analysis strategy are needed to search these "two kinds of leptons". For the search described in this theses, only $\tau$ of the first type, i.e. coming from the decay of $D$ and $B$ mesons, have been used.

Tau leptons can, not only originated from the decay of D mesons produced "promptly", but also from the D mesons produced from the decay of a B meson, which are listed separately in the table. These “indirect” decays can be distinguished from the “direct/prompt” ones (from directly produced D mesons) because they are characterized by larger transverse momenta and displaced vertices. // as it will be shown later...

The relative $\tau$ yields from the different mother-mesons are listed in Tab. \ref{tab:relativeTauYield}.\\
...
The $D_{s}^{\pm}$ mesons decays in $c\hat{s} \rightarrow W^{*} \rightarrow \tau + \nu_{\tau}$ and charge conjugate (CC), have a branching fraction of $\mathcal{B}(D_{s} \rightarrow \tau + \nu_{\tau})$ = 5.48\% \cite{ref112}. These decays are very similar to the decay of pions : $\pi \rightarrow \mu \nu_{\mu}$.\\
The $D^{\pm}$ mesons decays: $c\hat{d} \rightarrow W^{*} \rightarrow \tau + \nu_{\tau}$ (and CC) are suppressed by the Cabibbo angle and very little phase space available, because $m_{D}^{+} - m_{\tau} \sim$ 100 MeV). $D_{0}$ mesons ($c\hat{u}$ and CC) cannot decay to $\tau$ leptons (with only one EWK vertex involved) since at least one more charged Kaon/pion ($c \rightarrow (s/d)W^{*}$) needs to be produced in such a decay, which is not kinematically possible. Indeed, $m_{D_{0}} (1,865) <
80 m_{\tau} (1,777) + m_{\pi^{\pm}} (140)$,where particles’ masses shown in parentheses are given in MeV.

$B$ mesons decay directly to $\tau$ leptons via $b \rightarrow cW^{*} \rightarrow c(\tau \nu_{\tau})$ with the branching fraction of 3\% \cite{ref112}. Decays $b \rightarrow uW^{*}$ are highly suppressed by the small CKM mixing angle between 1st and 3rd fermion generations. All $B$ meson decays almost always produce $D$ mesons, some of which in turn can decay to $\tau$ leptons, as described above.


\subsection{Signal acceptance at generator level}

Challenge for CMS: muons from the $\tau \rightarrow 3\mu$ decays, with $\tau$ coming from B and D mesons, have very low momenta and are significantly boosted in the forward direction, as it is possible to see in Fig. \ref{fig:Momentum_genLevel}. In particular, even if the mother particles have different kinematics, the difference in momentum of $\tau$ leptons is barely visible, despite $B$ mesons are quite heavier than $D$ mesons.\\
Figure varie... 
...
The CMS fiducial volume to reconstruct muons is $|\eta| <$ 2.4 and $p >$ 2.5 GeV (muons of momentum less than 2.5 GeV have no chance at all to reach the Muon chambers) - REF. From Fig. varie \ref{fig:Eta_genLevel}, it can be seen that only about 2\% generator level events are in this fiducial volume.
...
To better illustrate the effect of muon momentum or transverse momentum cut on signal acceptance, Fig. \ref{fig:Efficiency_genLevel} plots efficiencies as a function of cut on p or pT of all 3 muons, while all muons have $|\eta| <$ 2.4.
...
\subsection{Datasets}
For the analysis described in this thesis, I have used data collected by CMS in pp collisions at a center of mass energy $\sqrt{s}$ = 13 TeV during Run II in the whole 2017 data-taking. During 2017 there have been 5 running periods labeled from B to F. \\
As already said in \ref{WLCG}, the data recorded by CMS are stored in several different primary datasets, depending on the trigger with which they were collected.
...
\begin{table}[h!]\footnotesize
	...
	\begin{tabular}{|l|c|c|}
	\hline 
	2017 Datasets 	& 	Run range	&	Integr. Luminosity ($fb^{-1}$)\\ \hline \hline
...
Only runs that satisfy the data quality criteria, ensuring that they were recorded when all detector subsystems were at the nominal operation conditions, which are listed in a file in the JSON format, were used in the analysis. The json file used for data processing is: 
Collisions17/13TeV/ReReco/Cert\_294927-306462\_13TeV\_EOY2017ReReco\_Collisions17\_JSON\_v1.txt.

- Devo elencare già qui gli L1 e HLT trigger ?

\subsection{MonteCarlo samples}
The MC samples, used to model signal and normalization processes, that have been used in this analysis are listed in Tab. \ref{tab:MCsamples}.\\
They are official CMS productions and they simulate $\tau$ coming from $D_{s}$, $B^{\pm}$ and $B^{0}$ decays, which correspond to  $sim$94\% of total $\tau$ production, as shown in \ref{tab:relativeTauYield}.
...
\begin{table}[h!]\footnotesize
	...
	$D_{s} \rightarrow \tau \nu_{\tau} \rightarrow 3\mu \nu_{\tau}$	&	/DsToTau_To3Mu\_MuFilter\_TuneCUEP8M1\_13TeV-	&	\\
		& pythia8/RunIIFall17DRPremix-PU2017\_94X\_mc2017\_	&	\\
			&	realistic\_v11-v1/AODSIM	&	$3.6\ \cdot\ 10^{6}$\\
	$B^{0}\rightarrow\tau \nu_{\tau}\rightarrow 3\mu \nu_{\tau}$	&	/BuToTau\_To3Mu\_MuFilter\_TuneCUEP8M1\_13TeV-x	&	\\
		& pythia8/RunIISummer16DR80Premix- PUMoriond17\_	&	\\
			&	80X\_mcRun2\_asymptotic\_2016_TrancheIV\_v6-v1/AODSIM	&	000\\
	$B^{\pm}\rightarrow\tau \nu_{\tau}\rightarrow 3\mu \nu_{\tau}$	&	/BdToTau\_To3Mu\_MuFilter\_TuneCUEP8M1\_13TeV-	&	\\
		& pythia8/RunIISummer16DR80Premix PUMoriond17\_	&	\\
			&	80X\_mcRun2_asymptotic\_2016\_TrancheIV_v6-v1/AODSIM	&	000\\
	$D_{s} \rightarrow \phi \pi \rightarrow 2 \mu \pi$	&	-	&	\\
		& -	&	000\\
	\hline
	\end{tabular}
...
Dalla nota:\\
The samples were generated using PYTHIA 8 \cite{ref104} with a MinimumBias configuration, and the decay chains are performed by EVTGEN \cite{ref110}  package. As for $B^{+}$ and $B^{0}$ production, a $b$-quark filter is applied on top of the MinimumBias configuration: an event is kept if there is a $b$-quark at the parton level, otherwise it is thrown away before hadronization. The decay chain is programmed using the EVTGEN as following - taking $D_{s} \rightarrow \tau$ as an example - if a $D_{s}$ meson is found after hadronization, it is forced to decay to a $\tau$, which is subsequently set to decay to 3$\mu$. In case multiple $D_{s}$ in one event, only one such decay chain is allowed. If no $D_{s}$ is found, the event is discarded. The $tau \rightarrow 3\mu$ process is asummed to be 3-body phase-space decay.\\ 
...
The analysis does not rely on the knowledge of absolute cross sections for $D_{s}$ and $B$ meson production. The signal event rate associated with hadronically produced $\tau$-leptons is derived from measurements of $D_{s} \rightarrow \phi \pi \rightarrow \mu\mu\pi$ production rate. For such a conversion, we use a number of decay branching fractions, which are shown in Tab. \ref{tab:MesonsBR} together with their uncertainties. 
...
from Raffa translated - In MC simulations is important to consider the pile up effect, that is present in each event in CMS (as already explained in \ref{LHC}).
The MC have been produced with a certainn pile-up distribution that, however, is different from the real one, id est the one of the collected data, as is possible to notice in Fig. \ref{fig:PU_comp}.
...
It is therefore necessary to correct the MC events in such a way they can simulate the real data conditions. This can be done by applying a proper weight to each event before any selections, both for data and MC.
After having normalized both distributions to 1, the weight, computed bin-by-bin (the bin is labelled with the index i), can be obtained from eq. \ref{eq:PU_rew}.
...
The effect of the pile-up reweighting is visibile especially in the distribution of the reconstructed vertices, as it is shown in Fig. \ref{fig:PU_Vertices}.
...
\begin{minipage}[b]{7.3cm}
 ...
 \end{minipage}
    \caption{N. of primary vertices in MC before (on the left) and after (on the right) the pile-up reweighting, compared to the same quantity in data.}
	\label{fig:PU_Vertices}
 \end{figure}

\section{Search strategy}
\label{strategy}
The events with $D$ and $B$ mesons decaying in a $\tau$ lepton that in turn decays in 3 muons, are characterized by low $\mathrm{p_{T}}$ muons, therefore the selection of the muon candidates must be optimised for muons with these characteristics, but at the same time they should reject the $\pi$ and $K$ mesons, which can be misidentified and recostructed as low $\mathrm{p_{T}}$ muons or the muons coming from the $K$ and $\pi$ decays in flight.
...
The production cross section of $D$ and $B$ mesons in the kinematic phase space relevant for the search is not know a priori, therefore it is obtained directly from data by measuring the event rate coming from $D_{s} \rightarrow \phi\pi \rightarrow(\mu\mu)\pi$ decays.\\

The signal is expected to form a narrow peak at the $tau$ mass, over a smoothly distributed background, in the trimuon mass distribution.\\
The analysis does not assume a priori knowledge of the nature of background and, in order to suppress it, a multi-variate analysis (MVA) is performed with a discriminant trained to distinguish signal events from background. To train the discriminant, for signal we rely on a simulated sample, while for background we use data events observed in the trimuon distribution sidebands, outside the region where the signal is expected to appear.

In order to improve the search sensitivity, all events that pass come "basics" selections, before being passed to the MVA, are sorted into 3 exclusive categories based on their expected trimuon mass resolutions.
After the MVA, the three trimuon mass-resolution categories are divided into three sub-categories based on the expected per-event S/B ratio as predicted by the MVA discriminant, the best two of which are then retained for the further statistical analysis. Finally, the signal is searched by simultaneously analyzing the thus-formed 6 unbinned mass distributions.
...
%	\label{tab:NEvents}
%\end{table}
In order to be kept, data should satisfy different requirements, which are organized in 2 levels, that I will call "Pre-selections" and "Selections".

The Pre-selections applied to choose the interesting $D_{s} \rightarrow \tau \nu \rightarrow
3 \mu \nu$ events are the following:
\begin{enumerate}
	\item The event has fired at least one among the following HLT paths:
		\begin{itemize}
		...
		\end{itemize}
	\item The events must have at least a triplet of muons with the following characteristics:
		\begin{itemize}
			\item $\mathrm{p_{T}\ >\ 0.5}$,
			\item $|\eta|\ <\ 2.4$,
			\item at least 1 hit in the inner tracker,
			\item the total electric charge of the triplet must be different equal to $pm$ 1,
			\item there must be at least one hit in the pixel detectors of the inner tracker.
		\end{itemize}
\end{enumerate}

After these very basic pre-selections, additional selections are applied in order to select only the triplets of interest for this analysis:
\begin{enumerate}
	\item The $\chi^2$ of the triplet vertex is in (0-15)
	\item The 3 muons are \emph{Global Muons} (REF)
	\item The 3 possible pairs of mu of the triplet have $\Delta R <$ 0.8 (REF)
	\item The 3 possible pairs of mu of the triplet have $|\Delta Z| <$ 0.5 (cos'è Delta Z)
	\item A veto on mass of opposite sign muons of the triplet compatible with $\phi$ meson
	\item A veto on mass of opposite sign muons of the triplet compatible with $\omega$ meson
	\item Mass window ??
	\item L1 ???
	\item trigger matching requirements
\end{enumerate}
If more than one triplet per event survives these selections (which happens very rarely, $\sim$ 1\% of the cases), it is chosen the one with the best $\chi^2$ value.

- Background:\\
To obtain an insight into the major source of background passing the described event selection criteria, a minimum bias sample of events have been simulated. N. events that pass all selection criteria are mostly due to B meson production with dimuon cascade decays of one the two $B$ mesons. The third reconstructed muon in an event is typically associated with a pion/Kaon faking a muon, half of the time via decaying to a muon in flight.


\newpage
\subsubsection*{Event categorization}
As already said in \ref{strategy}, in order to improve the search sensitivity, all events selected in the previous steps are classified into 3 exclusive categories based on their expected trimuon mass resolutions.\\
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=7.2cm]{MassTripReso}
...
\begin{figure}[h!]
  \centering
   \includegraphics[width=7.2cm]{MassTripReso_eta}
...
It is evident from Fig. \ref{fig:MassTripReso_eta} that the resolution of the mass of the triplet strongly depends on whether the trailing muon of the triplet (i.e. the muon with the lowest $mathrm{p_{T}}$ value is in the central or forward regions.

\newpage
\section{Signal Normalization}
\subsection{Events triggered by DoubleMu L1 seeds}
Dalla nota-\\
First, we consider signal events that are expected to be triggered by DoubleMu L1 seeds, about 75\% of all signal events according to MC. We start from the dominant source of $tau$ lepton, $D_{s} \rightarrow \tau$. $D_{s} \rightarrow \phi(\mu\mu)\pi$ channel is used to extract the total $D_{s}$ number from data. Besides, it moniters the perforamace of the L1 and high level trigger, as this $D_{s} \rightarrow \phi(\mu\mu)\pi$ events are collected with the same trigger path as the signal channel. In addition, by comparing kinematics of this channel in data and MC, it can be validated how well signal MC models data.
...
In these two equations, $\mathcal{L}$ is the integrated luminosity, $\sigma(pp \rightarrow D_{s})$ is the $D_{s}$ production cross section, $\mathcal{B}(D_{s} \rightarrow \tau \nu)$, $\mathcal{B}(\tau \rightarrow 3\mu)$, $\mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)$ are decay branching fractions, $\mathcal{A}$ is the event selection acceptance, $\epsilon_{trig}$ and $\epsilon_{reco}$ are instrumental trigger and reconstruction efficiencies.

By measuring $D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi$ decay rate $N$, one can predict $N_{sig}$ with significantly reduced systematic uncertainties:
...
The selections in $D_{s} \rightarrow \phi(\mu\mu)\pi$ are almost identical as those described in section \ref{Event_selection}, with the exception that, instead of three, two muons are required.
The 2 muons must be of opposite charge, and the invariant mass in the region of [1.00, 1.04] GeV, consistent with $\phi$ meson mass.
One additional track, pT $>$ 2 GeV, should be found to fit a secondary vertex together with the 2 muons, with normalized Chi2 of the vertex fitting smaller than 5. The quality requirements on this track include: at least 6 layers tracker measurements; at least 1 layer pixel measurements; transvere and longitudinal impact paramter with respect to the beam spot should be smaller than 0.3 cm and 20 cm respectively.

PRE-selections applied on the events:
\begin{enumerate}
	\item The event has fired the HLT path: \emph{HLT\_DoubleMu3\_Trk\_Tau3mu\_v*}
	\item There must be at least 1 triplet containing:
		\begin{itemize}
			\item 2 muons with the following characteristics:
				\begin{itemize}
					\item $\mathrm{p_{T}\ >\ 0.5}$, 
					\item  $|\eta|\ <\ 2.4$, 
					\item there must be at least 1 hit in the inner tracker,
					\item the electric charge must be different from 0,
					\item there must be at least one hit in the pixel detectors of the inner tracker.
				\end{itemize}
			\item 1 track with the following characteristics:
				\begin{itemize}
					\item $\mathrm{p_{T}\ >\ 2}$, 
					\item $|\eta|\ <\ 2.4$, 
					\item the electric charge must be different from 0,
					\item there must be at least 6 hits in the inner tracker,
					\item there must be at least one hit in the pixel detectors of the inner tracker.
				\end{itemize}					
		\end{itemize}
	\item Moreover, the triplet must have:
		\begin{itemize}
			...
			\item total electric charge = $\pm$1.
		\end{itemize}			
\end{enumerate}
...
After selections, the ”2 muon and 1 track” invariant mass region of [1.68, 2.02] GeV is used to extract $D_{s}$ yields. The distribution is fitted with 2 Crystal Ball functions and an exponential function, as shown in Fig. \ref{fig:NDs_total}. The smaller bump at 1.87 GeV is $D^{\pm}$. (For information, $\mathcal{B}(D^{+} \rightarrow \phi\pi \rightarrow \mu\mu\pi$ is 1.55 $ctimes\ 10^{-6}$).
...
The same fit but for each 2017 run era separately, is shown in Fig. \ref{fig:NDs_runByrun}, while the fitted $D_{s}$ yields from data, normalized to integrated luminosity, compared with MC predictions, are listed in Tab. \ref{tab:N_Ds}.\\
Dramatic fluctuation is observed over the year - problema run B e F !!!
...
\newpage
- Dalla nota\\
In order to further study the $D_{s}$ yields instability, the 3$\mu$ sideband yields are compared with the $D_{s}$ yields. The 3$\mu$ sideband events should pass the preselections in Section 5, and in addition,  the 3$\mu$ vertex is asked to be at least 3 sigma displaced from the primary vertex (so they are even more signal-like). Only events firing DoubleMu L1 seeds (L1 DoubleMu0er1p6 dEta Max1p8 OS or L1 DoubleMu 10 0 dEta Max1p8) are under study here. Figure 10 (left) shows the $D_{s}$ yields and 3$\mu$ sideband yields over the year. The same pattern is found for their variation. The data era B to E have only about half event rates, with respect to those in data era G and H. A mixed effect of Tracker APV issue and EMTF issue is visible. The latter was fixed during era E, which introduces a jump at data era F. The Tracker APV issue was fixed during era F, that is why rates in era G and H largely increased.\\
Nevertheless, if a ratio of 3$\mu$ sideband yields and $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields is made, as shown in Figure 10 (right), it is rather stable over the whole 2016. 
Using the Scale-Factor Method \cite{ref112}, the uncertainty assocaited with the fluctuation is found to be 3.3\%
($\sqrt{\chi^{2}/(nd\ f-1)} \cdot \Delta p_{0}/p_{0}$ = 3.3\%, based on the linear fit parameters). 
This is taken as a measure of systematics accociated with the $D_{s}$ normalization.

Fig. \ref{fig:Systematics_Ds} e Fig. \ref{fig:Systematics_Ds_ratio}.
...
To summarize the observations from Figure 10 and Figure 11: 
\begin{itemize}
	\item  3$\mu$ sideband and $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields ratio is stable. 
	\item The inefficiency in the first half of 2016 data does not significantly depend on a certain kinematical variable. Therefore we don’t apply kinematic reweighting to data B to F. The overall scale factor 0.67 (Table 9) is used to correct the $D_{s}$ yield difference between data and MC, and a 3.3\% uncertainty is assigned to this scale factor based on 3$\mu$ sideband and $D_{s}$ ratio seen above. In addition, we include the uncertainties on decay branching ratios involved in the normalization procedue, which are $\mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)$ (8\%) and
$\mathcal{B}(D_{s} \rightarrow \tau\nu$) (4\%). This overall scale factor is applied to signal events firing DoubleMu l1 seeds, which is 75\% of the total.
\end{itemize}
...
Dalla nota-\\
Direct $B \rightarrow \tau + ...$ decays are the second largest source of $\tau$-leptons. Their contribution to the signal can be written as reported in eq. \ref{eq:N_sigB}.
...
It cannot be directly normalized by $D_{s} \rightarrow \phi(\mu\mu)\pi$. 
Nevertheless, we can indirectly check if the ratio of $B$ meson and $D$ meson production rate in data is consistent with the MC prediction. 
The $D_{s}$ production is a mixture of prompt $D_{s}$ and $B \rightarrow D_{s}$. 
These two components are so different in terms of decay length. Figure 12 shows $D_{s} \rightarrow \phi(\mu\mu)\pi$ proper decay length, $L \cdot M/p$, where $L$ is the displacement of the $D_{s}$ vertext to the primary vertex in the laboratory frame, $M$ is the invariant mass of $\mu\mu\pi$, and p is the momentum of the $\mu\mu\pi$ system. The $D_{s} \rightarrow \phi(\mu\mu)\pi$ data distribution is mass-sideband subtracted. (Only data G and H are used, and only when the L1 seeds used were not pre-scaled.) 
...
By combining Eqs. \ref{eq:N}, \ref{eq:N_sigB}, \ref{eq:f}, the predicted contribution of direct $B$-decays to the signal is:
...
\subsection{Events triggered exclusivly by the TripleMu L1 seed}
???
...
From Nota:\\
The $D_{s}\rightarrow\phi(\mu\mu)\pi$ channel is also used to validate how well MC models data. Only data 2016 G and H are used to compare with MC, because of trigger inefficiency in data B to F. Moreover, only data events triggered when neither L1 DoubleMu 10 0 dEta Max1p8 nor  L1 DoubleMu0er1p6 dEta Max1p8 OS was pre-scaled are used here.
For a certain distribution x, data events having ”2 muon and 1 track” invariant mass in (1.93,
347 2.01) GeV are taken as $D_{s}\rightarrow\phi(\mu\mu)\pi$ candidates, the distribution of which is xpeak. On the other hand, data events in (1.70, 1.80) GeV region are used as sideband events, and their distribution is xsideband. Then xsideband is scaled, according to the fitted background shape (Figure 9), to the number of expcted background events in (1.93, 2.01) GeV region, before xsideband is subtracted from xpeak. In this way, the non-$D_{s}$ contamination is subtracted. The data comparison with $D_{s}\rightarrow\phi(\mu\mu)\pi$ MC is shown in Figs. \ref{fig:ControlPlots_1} and \ref{fig:ControlPlots_2}. \\
(The $D_{s}$ MC includes prompt $D_{s}$ and $B \rightarrow D_{s}$.) 

It is found that for all distributions MC agrees reasonably well with data.
...
\begin{figure}[h!]
 \begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_pTmu1}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_etaMu1}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_pTmu2}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_etaMu2}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_pTpi}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_EtaTriplet}
 \end{minipage}
...
\begin{figure}[h!]
 \begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_DecayLength}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_DecayLengthSign}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_DeltaR_2mu}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_segmComp}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{Control_other1}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{Control_other2}
 \end{minipage}
 \caption{Control plots variiii.}
...
It is evident that rectangular and diagonal cuts are not as good as nonlinear boundaries in classifying the events. In general, the best decision boundary is a surface in the $n$-dimensional space of input variables, which can be represented by an equation of the form $y(\bm{x}) = y_{cut}$, where $y_{cut}$ is some constant. Events are classified as signal if they are on one side of the boundary. For example $y(\bm{x}) \leq y_{cut}$, could represent the acceptance region and $y(\bm{x}) > y_{cut}$ could be the rejection region.\\
%- oppure "scalar test" statistics (vedi 14-15 Cowan per more details )

\subsection{Boosted Decision Tree (BDT)}
\subsubsection*{Decision Tree}
A \emph{decision tree} is a classifier structured as a binary tree, defined by a collection of successive cuts on the set of input variables. It is schematically represented in Fig. \ref{fig:DecisionTree}.\\
\begin{figure}[h]
	\centering
...
\end{figure}
Starting from the entire sample of training events in the root node, one finds the variables, among all of the possible input ones, that provide the best separation between signal and background by use of a single cut. Then, repeated left/right (yes/no) decisions are taken on one single variable at a time until a stop criterion is fulfilled. 
...
In a Boosted Decision Tree the idea previously explained is extended from one tree to several trees, which form a \emph{forest}. 
These trees are derived from the same training ensemble by reweighting events (this procedure is called \emph{boosting}), and are finally combined into a single classifier which is given by an average of the individual decision trees.
...
In particular, the performances are really improved if the boosting is applied to trees that have not much classification power if taken individually (called \emph{weak classifiers}).

Several boosting algorithms have been developed and they differ primarily in the rule used to update the weights. The one employed in the BDT used in this analysis is the so-called \emph{AdaBoost} (Adaptive Boost), which is explained in detail in \cite{ref117a}.

\subsection{Training, variable ranking, performance }
A MVA classification consists of two independent phases: the \emph{training phase}, where the multivariate methods are trained, tested and evaluated, and an \emph{application phase}, where the chosen methods are applied to the concrete classification they have been trained for.

The training of a decision tree is the process that defines the splitting criteria for each node.
The split is determined by finding the variable and corresponding cut value that provides the best separation between signal and background. The node splitting stops once it has reached the minimum number of events which is specified in the BDT configuration (option nEventsMin). \\
...
A point in favor of BDT is also the fact that they are insensitive to the inclusion of poorly discriminating input variables (which is not the case of other classifiers, as neural networks, for example). \\
Moreover they are relatively insensitive to the number of input variables used. Components that provide little or no separation between signal and background are rarely chosen in the spiltting of the tree, and thus they are effectively ignored. Furthermore, BDT are surprisingly insensitive to overtraining. That is, although the error rate on the test sample will not decrease to zero as one increases the number of boosting iterations (as is the case for the training sample), it tends not to increase. Further discussion of this point can be found in \cite{ref117c}.
...
At this stage, all data and signal MC events have passed the basic selections in section 5. Signal MC is normalized as described in section 6.
The data and MC events are given to BDT to further separate the two. 10 variables are used to train the BDT.

Kinematics:
\begin{itemize}
	\item p: The momentum of the lowest momentum muon in the event
\end{itemize}
Muon identificaiton: (Each variable has 3 values from the 3 muons, of which the ”worst” is used)
\begin{itemize}
	\item cLP: Inner track and muon outer track position matching Chi2
	\item tKink: The ”kink” of the inner track (a sign of light flavor decay-in-flight)
	\item segmcomp: Inner track and muon segment position compatibility
\end{itemize}
Vertexing and displacement:
\begin{itemize}
	\item fv\_nC: The normalized Chi2 of the 3$\mu$ vertex fitting
	\item fv\_dphi3D: The 3D angle between the 3$\mu$ momentum vector and the Vertex(3$\mu$)-PV
vector
...
\end{itemize}	
Isolation (i.e. additional tracks close to the 3$\mu$ candidates):
\begin{itemize}
	\item mindca iso: The closest distance of the 3$\mu$ vertex to any other track in the event (pT$>$1GeV)
	\item trkrel: The ratio of the pT of surounding tracks to that of the muon. Tracks are counted if pT $>$ 1 GeV, deltaR $<$0.3 and distance of the closest approach smaller than 1 mm to the muon. The largest value from the three muon is used
\end{itemize}

Fig. \ref{fig:BDT_inputs} shows the signal and background (3$\mu$ sideband in data) comparison of all these 10 variables.
...
About N. signal MC events and N. data sideband events have passed the selections in section 5. Half of these events are used to train BDT, and BDT outputs are compared with the other half of events. The BDT output distribution should be consistent between the 2 halves (known as ”training sample” and ”test sample”), to avoid ”overtraining”, as checked in Figure 21. No sign of overtraining is seen. 

These input values are sorted in a "ranking" based on (vedi spiegazione su) and they are listed in Tab. \ref{tab:BDT_ranking}.\\
The matrices reporting the linear correlations among the input variable are shown in Fig. \ref{fig:CorrelationMatrix}, both for the signal and the background.
...
Techinical parameters used in the BDT training are listed in Tab. \ref{tab:BDT_parameters}.
...
The three trimuon mass resolution components (REF) are hereinafter referred to as event category A, B and C respectively, from the best to the worst mass resolution.\\
Fig. \ref{fig:BDT_out} shows the BDT output of signal and background in the three categories separately. Cuts are applied on BDT outputs to improve signal background ratio, before the final results are extracted. Two cuts are made to each category, as indicated by the two vertical bars in each figure. Events sitting on the right to the right hand side vertical bar are referred to as ”sub-category 1”, while events between the two vertical bars are referred to as ”sub-category 2”. The two cuts are scanned simultaneously such that the best combined signal significance is obtained in each category (An optimization based on the best combined upper limit leads to the same cut values). As a result, we will have six categories in total, three mass resolution categories, times two BDT output categories.
...
\begin{minipage}[b]{7.3cm}
 ...
 \end{minipage}
 \caption{Dalla nota - Trimuon mass distributions in the six independent event categories used in the analysis. The six event categories are defined in the text. Data are shown with points. The background-only fit and the expected signal for $\mathcal{B}(\tau \rightarrow 3\mu) = 10^{-7}$ are shown with lines. -}
\label{fig:Final_cat}
\end{figure}
...
Dalla nota-\\
The dominant systematics is the uncertainty on signal normalization. The normalization procedure is detailed in Section 6 and briefly summarized here. The signal prediction is first taken from MC, and then the overall scale factor 0.67 is applied. Depending on the mother particle of the $\tau$ ($D$ source or $B$ source), further corrections are applied and systematics are assigned, based on Eq. \ref{eq:N_sigD_2} and Eq. \ref{eq:N_sigB_2} respectively. Depending on if the event is triggerred exclusively by the TripleMu L1 seed, the corrections and systematics discussed in Section 6.3 are applied.\\
Below is a full list of systematics.

- Uncertainty on $D_{s}$ normalization is 3.3\%. They apply to all events, either $D$ source or $B$ source, either triggerred by DoubleMu L1 seeds, or TripleMu L1 seed.
