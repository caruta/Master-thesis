...
The MC samples listed in Tab. \ref{tab:MCsamples} were generated using PYTHIA 8 \cite{ref104} with a MinimumBias configuration, and the decay chains are performed by EVTGEN \cite{ref110} package.\\ 
As for $B^{+}$ and $B^{0}$ production, a $b$-quark filter is applied on top of the MinimumBias configuration: an event is kept if there is a $b$-quark at the parton level, otherwise it is thrown away before hadronization. The decay chain is programmed using the EVTGEN as following - taking $D_{s} \rightarrow \tau$ as an example - if a $D_{s}$ meson is found after hadronization, it is forced to decay to a $\tau$, which is subsequently set to decay to 3$\mu$.\\ 
In case multiple $D_{s}$ in one event, only one such decay chain is allowed. If no $D_{s}$ is found, the event is discarded. The $\tau \rightarrow 3\mu$ process is asummed to be 3-body phase-space decay.\\ 
...
Da NAdia:\\
Generated 161k events $D_{S} \rightarrow \phi\pi \rightarrow (2\mu)\pi$ with PYTHIA 8, strating from a Minimum Bias event configuration and requiring at least one $D_{S}$ meson per event.
The $D_{S} \rightarrow \phi\pi$ and $\phi \rightarrow \mu\mu$ decays are then forced with BR=1.  Standard CMS event reconstruction algorithm are then applied to the final state particleas as described in section REF.

The 2017 realistic data taking conditions are simulated,
...
In particular, for the the distribution of a certain variable $x$, data events having ”2 muon and 1 track” invariant mass in (1.93-2.01) GeV are taken as $D_{s}\rightarrow\phi(\mu\mu)\pi$ candidates, the distribution of which is called $x\_SGN$. On the other hand, data events in (1.70, 1.80) GeV region are used as sideband events, and their distribution is called $x\_SB$. 

The normalization procedure is described in the following and that consists of three steps:
\begin{enumerate}
	\item \emph{MC normalization}:\\ The MC distribution of the $x$ variable is scaled of a factor $f_{MC}$:
		\begin{equation}
		f_{MC} = \frac{\mathcal{L} * \sigma(pp \rightarrow D_{s}) * \mathcal{B}(D_{s}\ , \rightarrow \phi(2\mu)\pi)}{N_{MC}}\ ,
		\end{equation}
		where:
		\begin{itemize}
			\item $\mathcal{L}$ is the integrated luminosity of the data run considered (listed in Tab. \ref{tab:2017datasets}),
			\item  $\sigma(pp \rightarrow D_{s})= 1.17 \cdot 10^{10}$  is the $D_{s}$ production cross section used in the MC production,
			\item $\mathcal{B}(D_{s} \rightarrow \phi(2\mu)\pi) = 1.29 \cdot 10^{-5}$ is the BR of the process $D_{s} \rightarrow \phi(2\mu)\pi$,
			\item $N_{MC}$ = 1832741, is the total number of events in the MC.
		\end{itemize}
	\item \emph{Datasideband ($x\_SB$) normalization}:\\ rhe $x\_SB$ is scaled, according to the fitted background shape (Fig. \ref{fig:NDs_runByrun}), to the number of expected background events in (1.93, 2.01) GeV region:
		\begin{equation}
		...
		\end{itemize}
	\item \emph{Subtraction of $x\_SB$ from $x\_SGN$}:\\ the contribution of from non-$D_{s}$ to the area of $x\_SGN$ is subtracted.
\end{enumerate}
...
\begin{table}[h!]\footnotesize
	...
	$D_{s}$	&	2.18$\cdot 10^{10}$	&	3.36$\cdot 10^{-3}$	&	4138\\
...
\begin{itemize}
	\item the MC\_Weight for meson $X$ will be used to scale the signal coming from meson $X$ and it can been computed using the equation:
		\begin{equation}
		\mathrm{MC\_Weight} = \frac{\sigma(pp \rightarrow X)*\mathcal{B}(X \rightarrow \tau)*\mathcal{B}(\tau \rightarrow3 \mu)*\mathcal{L}}{N_{MC}}
		\end{equation}
...
\begin{equation}
N_{sig(D)} = \mathcal{L} * \sigma(pp \rightarrow D_{s}) \mathcal{B}(D_{s} \rightarrow \tau \nu_{\tau}) * \mathcal{B}(\tau \rightarrow 3\mu) * \frac{N. eventi iniz dati (??)}{N.events\ MC} = //
\label{eq:N_D}
\end{equation}
where:
\begin{itemize}
	\item $\mathcal{L}$ = 38 $fb^{-1}$ is the integrated luminosity of the data collected,
	\item $\sigma(pp \rightarrow D_{s})$ = 1.17$^{10}$ is the $D_{s}$ production cross section,
	\item $\mathcal{B}(D_{s} \rightarrow \tau \nu)$ = 0.055 is the BR of the $D_{s} \rightarrow \tau \nu)$ decay,
	\item $\mathcal{B}(\tau \rightarrow 3\mu)$ is the BR of the $\tau \rightarrow 3\mu)$ decay, assumed to be 10$^{-7}$,
	\item $N.events\ MC$ = 1832741 is the total number of events present in the MC $D_{s} \rightarrow \phi(\mu\mu)\pi$,
\end{itemize}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm]{BDT_in1_A.pdf}
%\end{figure}
%\begin{figure}[h]
%	\centering
	\includegraphics[width=15cm]{BDT_in2_A.pdf}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm]{BDT_KStest.pdf}
	\caption{BDT_KStest.}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm]{ROC_curve.pdf}
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=12cm]{BDT_out_A.pdf}
   \includegraphics[width=12cm]{BDT_out_B.pdf}
   \includegraphics[width=12cm]{BDT_out_C.pdf}
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=12cm]{BDT_2Dmap_catA.pdf}
   \includegraphics[width=12cm]{BDT_2Dmap_catB.pdf}
   \includegraphics[width=12cm]{BDT_2Dmap_catC.pdf}
    \caption{BDT_out_2D.}
