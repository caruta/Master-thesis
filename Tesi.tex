...
The search for $\tau \rightarrow 3\mu$ decays represents a challenge for CMS because muons resulting from the decay of $\tau$ leptons, coming from $D$ and $B$ mesons, have very low momenta and are significantly boosted in the forward direction. This is shown in Fig. \ref{fig:Momentum_genLevel} and \ref{fig:Mu_Momenta_genLevel}, where the $\mathrm{p_{T}}$ and the momenta of the three muon of signal, at generator level, are displayed. Moreover, in Fig. \ref{fig:Eta_genLevel}, 

\begin{figure}[h!]
   \centering
   \includegraphics[width=14cm]{Mu_Momenta_genLevel.png}
	\caption{Mu\_Momenta\_genLevel.}
	\label{fig:Mu_Momenta_genLevel}
\end{figure}

\begin{figure}[h!]
   \centering
   \includegraphics[width=14cm]{Mu_PT_genLevel.png}
	\caption{Mu\_PT\_genLevel.}
	\label{fig:Mu_PT_genLevel}
\end{figure}

\begin{figure}[h!]
   \centering
   \includegraphics[width=15cm]{Eta_genLevel}
	\caption{Pseudorapidity $\eta$ of the most central muon (left) and the most forward muon (right) in $\tau \rightarrow 3\mu$ decays, where $\tau$ is associated with directly-produced D mesons (solid histogram) and decays of B mesons (dashed histogram). For B mesons, both $B \rightarrow \tau\nu_{\tau} + D + ...$ and $B \rightarrow D(\tau\nu_{\tau}) + ...$ decays are included.}
	\label{fig:Eta_genLevel}
\end{figure}

In Fig. \ref{fig:Momentum_genLevel} and 
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=15cm]{Momentum_genLevel.png}
	\caption{Dalla nota - (Top) Distributions of (left) pT, (middle) p, and (right) $\eta$ for directly-produced D (solid histogram) and B (dashed histogram) mesons. (Bottom) Distributions of the same observables for $\tau$-leptons originating from directly produced D meson (solid histogram) and from directly produced B mesons (dashed histogram) For B mesons, both $B \rightarrow \tau\nu_{\tau} + D + ...$ and $B \rightarrow D(\tau\nu_{\tau}) + ...$ decays are included.}
	\label{fig:Momentum_genLevel}
\end{figure}

%\begin{figure}[h!]
%   \centering
%   \includegraphics[width=12cm]{DeltaR_genLevel.png}
%	\caption{DeltaR\_genLevel.}
%	\label{fig:DeltaR_genLevel}
%\end{figure}

The CMS fiducial volume to reconstruct muons is $|\eta| <$ 2.4 and $p >$ 2.5 GeV (muons of momentum less than 2.5 GeV do not reach the muon chambers), as explained in section \ref{Muon reconstruction}.\\ 
To better illustrate the effect of muon momentum or transverse momentum cut on signal acceptance, Fig. \ref{fig:Efficiency_genLevel} shows efficiencies as a function of cut on p or pT of all 3 muons, while all muons have $|\eta| <$ 2.4. It can be seen that only $\sim$2\% of generator level events are in the CMS fiducial volume.
...
Montecarlo samples are used to simulate the processes that contribute to the signal and for the estimation of the number of events expected in data.
The MC samples that have been used in this analysis are listed in Tab. \ref{tab:MCsamples}.\\
They are official CMS productions and they simulate the $\tau$ leptons coming from $D_{s}$, $B^{\pm}$ and $B^{0}$ decays, which correspond to $\sim$94\% of total $\tau$ production, as shown in Tab. \ref{tab:relativeTauYield}.\\
Moreover, also the $D_{s} \rightarrow \phi \pi \rightarrow (\mu\mu)\pi$ decay was simulated because this channel it is important both as a \emph{control channel} which can be used to validate the signal MC samples and also to monitor the performance of the trigger during the data-taking, since its events are acquired with the same trigger paths. More details on these will be given in section ref{sgn_norm}. 
...
\begin{table}[h]\footnotesize
	...
	$D_{s} \rightarrow \tau \nu_{\tau} \rightarrow 3\mu \nu_{\tau}$	&	/DsToTau\_To3Mu\_MuFilter\_TuneCUEP8M1\_13TeV-	&	\\
		& pythia8/RunIIFall17DRPremix-PU2017\_94X\_mc2017\_	&	\\
			&	realistic\_v11-v1/AODSIM	&	$1.2\ \cdot\ 10^{6}$\\ \hline
...
All MC samples listed in Tab. \ref{tab:MCsamples} were generated using PYTHIA 8 \cite{ref104} with a MinimumBias configuration, and the decay chains are performed by EVTGEN \cite{ref110} package.\\ 
In particular, to simulate different processes, several steps have been followed:
\begin{itemize}
	\item for the processes $B^{+} \rightarrow \tau \nu_{\tau}\rightarrow 3\mu\nu_{\tau}$ and $B^{0}\rightarrow \tau \nu_{\tau}\rightarrow 3\mu\nu_{\tau}$:
	\begin{enumerate}
		\item the $B^{+}$ and $B^{0}$ productions are simulated by applying a $b$-quark filter on top of the MinimumBias configuration: an event is kept if there is a $b$-quark at the parton level, otherwise it is thrown away before hadronization.
		\item After the hadronization, if a $D_{s}$ meson is found, it is forced to decay into a $\tau$, that is then forced to decay in 3$\mu$ (this last process is asummed to be three-body phase-space decay).
	\end{enumerate} 
	If there are more than one $D_{s}$ per event, only one such decay chain is allowed. These last steps (from point 2) are the same followed for the production of the $D_{s}\rightarrow \tau \nu_{\tau} \rightarrow 3\mu\nu_{\tau}$ MC sample.
	\item for the control channel $D_{s} \rightarrow \phi \pi \rightarrow \mu\mu\pi$ the procedure is similar, but the $D_{s} \rightarrow \phi\pi$ is set to be scalar to vector plus scalar decay, while $\phi \rightarrow \mu\mu$ is a vector to leptons decay with radiative correction. As for $D_{s}$ samples, both $D_{s}$ from prompt production and $b$-hadron decays are included.
\end{itemize}
Not all the events are processed with time consuming full simulation, but only the ones that have at the generator level at least $\mu$ with $\mathrm{p_{T}} >$ 2.7 GeV and $|\eta| <$ 2.45.\\ 


In order to compute the expected number of mesons contributing, through their decay, to the final signal, all the needed decay branching fractions with their uncertainties are reported in Tab. \ref{tab:MesonsBR}.
...
\begin{table}[h]\footnotesize
	...
	$D_{s}\rightarrow\tau \nu_{\tau}$	&	(5.48 $\pm$ 0.23)$\cdot 10^{-2}$	&	PDG \cite{ref112}\\
	$B^{+}\rightarrow\tau \nu_{\tau} D_{0}^{*}$	&	(2.7 $\pm$ 0.3)$\cdot 10^{-2}$		&	PDG \cite{ref112}\\
	$other B^{+}\rightarrow\tau \nu_{\tau} X$	&	0.7$\cdot 10^{-2}$	&	PYTHIA \cite{ref104}\\
	$B^{0}\rightarrow\tau \nu_{\tau} D^{+*}$	&	(2.7 $\pm$ 0.3)$\cdot 10^{-2}$		&	PDG \cite{ref112}\\
	$other B^{0}\rightarrow\tau \nu_{\tau} X$	&	0.7$\cdot 10^{-2}$	&	PYTHIA \cite{ref104}\\
	$B^{+}\rightarrow D_{s} X$	&	(9.0 $\pm$ 1.5)$\cdot 10^{-2}$	&	PDG \cite{ref112}\\
	$B^{0}\rightarrow D_{s} X$		&	(10.3 $\pm$ 2.1)$\cdot 10^{-2}$	&	PDG \cite{ref112}\\
...
\begin{figure}[h]
   \centering
   \includegraphics[width=15cm]{PU_comparison.pdf}
...
\begin{figure}[h]
   \centering
   \includegraphics[width=15cm]{Weight.pdf}
...
\begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=7.5cm]{NPrimaryVertices_Before.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{NPrimaryVertices_After.pdf}
 \end{minipage}
...
\begin{enumerate}
	\item The event must have fired at least one among the following L1 triggers:
		\begin{itemize}
			...
			\item \emph{L1\_TripleMu\_5\_3\_0\_DoubleMu\_5\_3\_OS\_Mass\_Max17}
			\item \emph{L1\_TripleMu\_5SQ\_3SQ\_0\_DoubleMu\_5\_3\_SQ\_OS\_Mass\_Max9}
		\end{itemize}
	\item The event must have fired the  HLT path: \emph{HLT\_DoubleMu3\_Trk\_Tau3mu\_v*},
	\item The event must contain at least a triplet of muons and each of them has to present the following characteristics:
		\begin{itemize}
			\item $\mathrm{p_{T}\ >\ 0.5}$ and $|\eta|\ <\ 2.4$,
...
\begin{enumerate}
	...
	\item the 3 possible pairs of muons of the triplet have $\Delta R <$ 0.8 (REF),
	...
	\item both L1 double and L1 triple triggers fired \& \emph{HLT\_DoubleMu3\_Trk\_Tau3mu\_v*} fired
...
\subsubsection{Analysis optimization and background studies}
These sets of selections were optimized using the MC simulation.\\

The results of a study to optimize the muon ID requirements for the 3 candidate muons is reported in Tab. \ref{tab:MuonID}.

\begin{table}[h]\footnotesize
	\centering
	\begin{tabular}{|l|c|c|c|c|c|}
	\hline
	Muon ID 	& 	N events MC &	N events MC & N events MC  & N events 2017 & S/N \\
		&	 $D^{\pm}_{S} \rightarrow \tau \nu_{\tau} \rightarrow 3\mu \nu_{\tau}$	&	$B^{0} \rightarrow \tau \nu_{\tau} \rightarrow 3\mu \nu_{\tau}$	&	$B^{\pm} \rightarrow \tau \nu_{\tau} \rightarrow 3\mu \nu_{\tau}$	&	data sidebands &	\\	\hline \hline
	3 global					&	61558	&	30920	&	44005	&	66823	&	2.04	\\
	2 global+1 tracker	&	78077	&	39954	&	57109	&	251059	&	0.70	\\
	2 global+1 loose		&	73337	&	37214	&	53432	&	129647	&	1.26	\\
	2 global+1 soft		&	77577	&	39638	&	56639	&	187768	&	0.92	\\
	3 soft						&	82169	&	42174	&	60321	&	241908	&	0.76	\\
	2 soft+1 track			&	82694	&	42496	&	60805	&	298921	&	0.62	\\
	3 loose						&	76193	&	38767	&	55482	&	141414	&	1.21	\\
	2 loose+1 soft			&	80438	&	41146	&	58706	&	185311	&	0.97	\\
	\hline
	\end{tabular}
	\caption{Muon ID optimization.}
	\label{tab:MuonID}
\end{table}

Inserisci anche sensitività scalando il segnale con BR Tau3Mu
...
%	\begin{tabular}{|c|c|}
%	\hline
%	PdgID	& 	Particle \\ \hline \hline
%	13			&	$\mu$	\\	
%	15			&	$\tau$	\\
%	\hline
%	\end{tabular}
%	\caption{Particle PDG ID.}
%	\label{tab:PDG_ID}
%\end{table}
...
In Fig. \ref{fig:Bkg_study2} is shown the PdgID of the particles that survive all the selection (listed before - REF) and the ones of their mother particle, in the case of the $D^{\pm}_{s} \rightarrow \tau\nu_{\tau} \rightarrow 3\mu\nu_{\tau}$ MC.\\
It is possible to see that:
\begin{itemize}
	\item in $\sim$99.50\% of the cases the muon comes from a tau lepton - GOOD!
	\item in $\sim$0.26\% of the cases there is a reconstruction error (PDGID = NN = 0)
	\item in $\sim$0.24\% of the cases the muon is not from a tau - BAD!
\end{itemize}

\begin{figure}[h!]
  \centering
   \includegraphics[width=15cm]{Bkg_study2}
    \caption{DsTau3Mu- AC}
	\label{fig:Bkg_study2}
\end{figure}

In Fig. \ref{fig:Bkg_study3} is shown the PdgID of the particles that survive all the selection (listed before - REF) and the ones of their mother particle, in the case of the $B^{0} \rightarrow \tau\nu_{\tau} \rightarrow 3\mu\nu_{\tau}$ MC.\\
It is possible to see that:
\begin{itemize}
	\item in $\sim$98.28\% of the cases the muon comes from a tau lepton - GOOD!
	\item in $\sim$0.38\% of the cases there is a reconstruction error (PDGID = NN = 0)
	\item in $\sim$1.34\% of the cases the muon is not from a tau - BAD!
\end{itemize}

\begin{figure}[h!]
  \centering
   \includegraphics[width=15cm]{Bkg_study3}
    \caption{B0Tau3mu - AC}
	\label{fig:Bkg_study3}
\end{figure}

In Fig. \ref{fig:Bkg_study4} is shown the PdgID of the particles that survive all the selection (listed before - REF) and the ones of their mother particle, in the case of the $B^{\pm} \rightarrow \tau\nu_{\tau} \rightarrow 3\mu\nu_{\tau}$ MC.
It is possible to see that:
\begin{itemize}
	\item in $\sim$98.76\% of the cases the muon comes from a tau lepton - GOOD!
	\item in $\sim$0.41\% of the cases there is a reconstruction error (PDGID = NN = 0)
	\item in $\sim$0.83\% of the cases the muon is not from a tau - BAD!
\end{itemize}

\begin{figure}[h!]
  \centering
   \includegraphics[width=15cm]{Bkg_study4}
    \caption{BpTau3mu - AC}
	\label{fig:Bkg_study4}
\end{figure}
...
Individual muon uncertainties on momentum measurement can be predicted on a per-muon level and it will then be propagated to the 3$\mu$ case to predict the mass error on an event-by-event basis.  (REF PAS H-4l)

To propagate uncertainties of individual muons to the uncertainty on the invariant mass of the triplet, each $\delta m$ corresponding to individual lepton momentum variation is calculated separately, as shown in eq. \ref{eq:delta_mi}.
\begin{equation}
\delta m_{i} = F(...; p_{Ti} + \delta p_{Ti}, \phi_{i}, \eta_{i}; ...) - m_{0}
\label{eq:delta_mi}
\end{equation}
with:

\begin{equation}
m_{0} = F(p_{T1}, \phi_{1}, \eta_{1}; p_{T2}, \phi_{2}, \eta_{2}; p_{T3}, \phi_{3}, \eta_{3})
\label{eq:m_0}
\end{equation}

Then the measured resolution on the invariant mass of the three leptons is taken as the quadrature sum of the three individual $\delta m$:
\begin{equation}
\delta m = \sqrt{\delta m_{1}^2 + \delta m_{2}^2 + \delta m_{3}^2}
\end{equation}

The triplet mass resolution is therefore given, event-by-event, by: $\frac{\delta m}{m}$.

The trimuon mass resolutions, both for MC and data, are shown in Fig. \ref{fig:MassTripReso}. It is evident that the distribution is characterized by three regions, delimited by $\delta m/m <$ 0.0070; 0.0070 $< \delta m/m <$ 0.0105; $\delta m/m >$ 0.0105, which will be used as limits for the three categories. The three trimuon mass resolution components are hereinafter referred to as event category A, B and C respectively, from the best to the worst mass resolution.

\begin{figure}[h!]
   \centering
   \includegraphics[width=16cm]{TripletMassReso.pdf}
    \caption{Mass resolution of the muon triplet per event in signal MC (blue filled area) and data events (black dots). Distributions are normalized to unity. The event categorization is indicated by vertical dashed red bars in the figure: category A: $\delta m/m <$ 0.0070; category B: 0.0070 $< \delta m/m <$ 0.0105; category C: $\delta m/m >$ 0.0105.}
...
\begin{figure}[h!]
  \centering
   \includegraphics[width=12cm]{MassTripReso_eta}
    \caption{Pseudorapidity value of the trailing (most forward) muon of the triplets, shown for the 3 different categories : category A is shown in blue, category B in red and category C in black.}
	\label{fig:MassTripReso_eta}
\end{figure}

\newpage
\section{Studies on the control channel and signal normalization}
\label{sgn_norm}
%- Selezioni DSPhiPi (preselzioni uguali sui primi 2 muoni)
%- Validazione MC -stressa importanza per MVA
%- Introduzionne stima sistematica : DS etc

this analysis the production cross section of $D$ and $B$ mesons in the kinematic phase space relevant for the search is not know a priori, but it is derived directly from data by measuring the $D_{s}$ production rate in the $D_{s} \rightarrow \phi\pi \rightarrow(\mu\mu)\pi$ channel.


At this point of the analysis, before proceeding with the use of a multivariate discriminator to further distinguish signal events from background ones, it is fundamental to validate the Montecarlo samples. In fact the $D^{\pm}_{s}$, $B^{0}$ and $B^{\pm}$ Montecarlo samples will be used, in the next analysis step, to train the classifier on the signal. For this reason they have to properly simulate the data and, if it is not the case, they must be corrected or reweighted in order to avoid that non-real effects present in the MC would affect and bias the discriminator.

In order to validate the MC samples used in the analysis, the decay channel $D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi$ has been used as a \emph{control channel}.\\
The importance of the study of this decay channel is twofold: it is not only necessary to validate the MC (because the its kinematics is very similar to the one that characterized the searched signal), but also it is useful to monitor the performance of the L1 and HLT triggers, because the $D_{s} \rightarrow \phi(\mu\mu)\pi$ events are collected with the same trigger path as the signal channel.

In order to validate the MC samples, the selections similar to the ones used in section \ref{Event_selection} have been applied on the $D_{s} \rightarrow \phi(\mu\mu)\pi$ MC and on the data collected with the L1_DoubleMu seed, with few differences for the requirements on the third muon that in this case is just a track.\\
In particular, the pre-selections applied on the events are the following:
\begin{enumerate}
	...
	\item The event must contain at least 1 triplet made up of:
		\begin{itemize}
			\item 2 muons with the following characteristics:
				\begin{itemize}
					\item $\mathrm{p_{T}\ >\ 0.5}$ and $|\eta|\ <\ 2.4$, 
					\item at least 1 hit in the inner tracker and 1 hit in the pixel detectors,
					\item electric charge different from 0,
				\end{itemize}
			\item and 1 track with the following characteristics:
				\begin{itemize}
					\item $\mathrm{p_{T}\ >\ 2}$ and $|\eta|\ <\ 2.4$, 
					\item electric charge different from 0,
					\item at least 6 hits in the inner tracker and 1 hit in the pixel detectors.
				\end{itemize}					
		\end{itemize}
...
After these pre-selections, another set of requirements is applied in order to select only the events that contain at least one triplet with the following characteristics:
\begin{enumerate}
...
The goodness of these previous selections can be verified looking at the which are the mothers of the particles that satisfy all of the requirements. 
In Fig. \ref{fig:Bkg_study1} is shown the PdgID of the particles that survive all the selection (listed before - REF) and the ones of their mother particle, in the case of the $D^{\pm}_{s} \rightarrow \phi(\mu\mu)\pi$ MC.\\

\begin{figure}[h]
  \centering
   \includegraphics[width=15cm]{Bkg_study1}
    \caption{DsPhiPi - After cuts}
	\label{fig:Bkg_study1}
\end{figure}

Therefore, after having applied to the data collected with the L1_DoubleMu seed, the selections listed before, the ”2 muon and 1 track” invariant mass region of [1.68, 2.02] GeV is used to extract $D_{s}$ yields. The mass distribution is fitted with 2 Crystal Ball functions and an exponential function, as shown in Fig. \ref{fig:NDs_total}. The smaller bump at 1.87 GeV correspons to the $D^{\pm}$ meson, which has a BR: $\mathcal{B}(D^{+} \rightarrow \phi\pi \rightarrow \mu\mu\pi)$ is 1.55 $\cdot \ 10^{-6}$.
...
The same fit was performed for each of the 2017 run era separately, and it is shown in Fig. \ref{fig:NDs_runByrun}. The fitted $D_{s}$ yields from data, normalized to integrated luminosity are compared with MC prediction and they are listed in Tab. \ref{tab:N_Ds}.\\
It can be noticed that the $D_{s}$ yield in data is compatible with the MC predictions in C, D and E runs, while there is a relevant discrepacy in run B and F.\\

Mettere un plot del fit MC ? (non scrivo da nessuna parte qual è lo yield delle Ds nel MC)?\\
Aggiungi Ds expected from MC/fb-1 nella tabella ??

\begin{table}[h]\footnotesize
	\centering
	\begin{tabular}{|c||c|c|}
	\hline
	Run 		& 	$D_{s}$ yields per $fb^{-1}$ in data	&	Data/MC ratio\\ \hline 
	2017 B	&	1022		&	0.366\\
	2017 C	&	2643	&	0.945\\
	2017 D	&	2724		&	0.974\\
	2017 E	&	2585	&	0.924\\
	2017 F	&	1779		&	0.636\\ \hline
	Whole 2017	&	2183 &	0.781\\
	\hline
	\end{tabular}
	\caption{Fitted $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields for each of the 2017 runs and ratio between the $D_{s}$ yield in data and in MC.}
	\label{tab:N_Ds}
\end{table}
...
In order to validate the MC samples, the $D_{s}\rightarrow\phi(\mu\mu)\pi$ MC is compared with the data after a procedure that normalizes the two distributions.\\
In particular, for the the distribution of a certain variable $x$, data events having ”2 muon and 1 track” invariant mass in (1.93-2.01) GeV are taken as $D_{s}\rightarrow\phi(\mu\mu)\pi$ candidates, the distribution of which is called $x\_SGN$. On the other hand, data events in (1.70, 1.80) GeV region are used as sideband (SB) events, and their distribution is called $x\_SB$. 

The normalization procedure is described in the following and that consists of three steps:
\begin{enumerate}
	\item MC normalization:\\ The MC distribution of the $x$ variable is scaled of a factor $f_{MC}$:
		\begin{equation}
		f_{MC} = \frac{\mathcal{L} * \sigma(pp \rightarrow D_{s}) * \mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)}{N_{MC}}\ ,
		\end{equation}
		where:
		\begin{itemize}
			\item $\mathcal{L}$ is the integrated luminosity of the data considered (listed in Tab. \ref{tab:2017datasets}),
			\item  $\sigma(pp \rightarrow D_{s})= 1.17 \cdot 10^{10}$  is the $D_{s}$ production cross section used in the MC production,
			\item $\mathcal{B}(\mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)) = 1.3 \cdot 10^{-5}$ is the BR of the process $\mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)$,
			\item $N_{MC}$ = 1832741 is the total number of events in the MC.
		\end{itemize}
	\item Data SB ($x\_SB$) normalization:\\ The $x\_SB$ is scaled, according to the fitted background shape (Fig. \ref{fig:NDs_runByrun}), to the number of expected background events in (1.93, 2.01) GeV region:
		\begin{equation}
		f_{dataSB} = Bkg_inSGNregion_MC * \frac{Bkg_inSB_MC}{Entries dataSB}\ ,
		\end{equation}
		where:
		\begin{itemize}
			\item $Bkg_inSGNregion_MC$ is the number of background events which are present in the "signal region", obtained from the previous fit...,
			\item $Bkg_inSB_MC$ is the number of background events which are present in the sideband region, obtained from the previous fit...,
			\item $Entries dataSB$ is the total number of events present in the $x\_SB$ distribution.
		\end{itemize}
	\item Subtraction of $x\_SB$ from $x\_SGN$:\\ The contribution of non-$D_{s}$ to the area of $x\_SGN$ is subtracted.
\end{enumerate}

The data comparison with $D_{s}\rightarrow\phi(\mu\mu)\pi$ MC is shown in Fig. \ref{fig:ControlPlots_1}. In particular, only data belonging to run era C of 2017 have been used because of trigger, as seen before, in this era the trigger perfomance/N Ds were very stable ??\\ 
In Fig. \ref{fig:ControlPlots_1} are displayed the transverse momenta and pseudorapidity value of the two muons and of the track, which are fundamental quantities to be checked also because I will use some of them or quantities related in order to train the BDT.\\
(The $D_{s}$ MC includes prompt $D_{s}$ and $B \rightarrow D_{s}$.) 

As it is evident, all the MC distributions agrees reasonably well with data; therefore the MC is validated!
...
\begin{minipage}[b]{7.7cm}
 ...
 \end{minipage}
 \caption{Control plots for the MC validation, comparing the distributions of transverse momentum and pseudorapidity for the 2 muons and the track that form the triplet. The MC distributions are displayed in blue, while the data distributions are in red.}
\label{fig:ControlPlots_1}
\end{figure}

\newpage
\subsection{Signal normalization}
The number of $D_{s}$ mesons that are expected (in teoria) to contribute to the signal (i.e. that are expected to decay in a $\tau$ lepton that in turn decays into three muons) can be evaluated using the cross section of the different processes, which are also used to produce the MC samples and are listed in Tab. \ref{tab:MC_Xsections}.
\begin{table}[h]\footnotesize
	\centering
	\begin{tabular}{|c|c|c|c|}
	\hline
	Meson $X$	& 	$\sigma(pp \rightarrow X)$ in MC	&	MC\_Weight for meson $X$	&	Expected number of $X$ mesons \\ \hline 
	$D^{\pm}_{s}$	&	2.18 $\cdot\ 10^{10}$	&	3.36 $\cdot\ 10^{-3}$	&	4138\\
	$B^{0}$	 &	9.37 $\cdot\ 10^{9}$	&	4.15 $\cdot\ 10^{-4}$	&	125\\
	$B^{\pm}$	&	9.22 $\cdot\ 10^{9}$	&	6.11 $\cdot\ 10^{-4}$	&	1230\\
	\hline
	\end{tabular}
	\caption{MC cross sections weights that have to be applied for the MC scaling for different processes. The expected number of mesons contributing to the signal is also computed.}
	\label{tab:MC_Xsections}
\end{table}

where:
\begin{itemize}
	\item the MC\_Weight for meson $X$ will be used to scale the signal coming from meson $X$ and it can been computed using the equation:
		\begin{equation}
		\mathrm{MC\_Weight} = \frac{\mathcal{L} * \sigma(pp \rightarrow X)*\mathcal{B}(X \rightarrow \tau)*\mathcal{B}(\tau \rightarrow3 \mu)}{N_{MC}}
		\end{equation}
		where $\mathcal{B}(X\rightarrow\tau)$ is the BR of the process $X\rightarrow\tau$, $\tau\rightarrow3\mu$ is assumed to be equal to $10^{-7}$ and $\mathcal{L} $ is the total integrated luminosity of the collected data (for 2017 it is 38$fb^{-1}$ and $N_{MC}$ is the total initial number of events in the MC before any selection.
	\item the expected number of $X$ mesons can be obtained using the equation:
	\begin{equation}
	\mathrm{Expected\ number} = \mathrm{MC\_Weight} *  N_{MC}.
	\end{equation}
\end{itemize}.
The expected number of mesons contributing to the signal that are listed in the table, must be than multiplied to the efficiency of the selections performed in the analysis.
Then the final number obtained for $D_{s}$ and $B$s contribution is:

NN

Actually, this is the number of mesons expected from the MC predictions.
However it is not guaranteed that this reflects properly the real conditions of data. In fact this number can be compared to the number of $D_{s}$ mesons measured in the data!	\\


In order to measure this value inn data, let's make some preliminary observations:
...
\begin{table}[h]\footnotesize
	...
	\begin{tabular}{|c||c|c|c|}
	\hline
	Run 		& 	SB data yields per $fb^{-1}$	&	SB data yields per $fb^{-1}$ in data	&	SB data yields per $fb^{-1}$ in data\\
				&	 for L1 DoubleMu seed			&	for L1 TripleMu seeds	&	for L1 DoubleMu OR TripleMu seeds \\ \hline 
	2017 B	&	286	&	374	&	489\\
	2017 C	&	918	&	1081	&	1465\\
	2017 D	&	906	&	1143	&	1508\\
	2017 E	&	906	&	1098	&	1445\\
	2017 F	&	711	&	867	&	1144\\ 
	\hline
	\end{tabular}
	\caption{Yields in 3$\mu$ data sidebamds for each of the 2017 runs in data collected with L1 DoubleMu seed, L1 TripleMu seeds and the OR of L1 DoubleMu and L1 TripleMu seeds.}
	\label{tab:L1_seeds}
\end{table}

Plot che mostra l'anfamento di questi dati e infine il rapporto (Only 3mu)/2mu L1 seeds nelle sidebands dei dati.

\begin{figure}[h!]
\begin{minipage}{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{L1_yields.pdf}
\end{minipage}
\begin{minipage}{7.5cm}
	\centering
	\includegraphics[width=7.5cm]{L1_ratio.pdf}
\end{minipage}
	 \caption{For each of 2017 runs, the yields for the different L1 seeds in 3$\mu$ SB in data are displayed: in blue --bla bla on the right. On the left is shown the ratio between the onnly TripleMu seeds and the Double Mu seed.}
	\label{fig:Systematics_Ds_ratio}
\end{figure}

\newpage
\subsection{Evaluation of systematics on the normalization}
In order to evaluate the systematics on the normalization, it is important to quantify the disagreement between the number of producted $D_{s}$ in data from the one which have been estimated from the MC cross section.
In order to further study the $D_{s}$ yields instability, the 3$\mu$ sideband yields are compared with the $D_{s}$ yields. The 3$\mu$ sideband events should pass the preselections in section \ref{Event_selection}.

Only events firing the DoubleMu L1 seed are under study here.\\ 
In tab. \ref{tab:Yields_Ratio} is reported also the 3$\mu$ sideband yields over the year, and it is compared to the $D_{s}$ yield in  Fig. \ref{fig:Systematics_Ds_ratio} (left). It is possible to see that the two yields have a similar trend, therefore the ratio between the sideband yields and the $D_{s}$ yield is displayed in Fig. \ref{fig:Systematics_Ds_ratio}, from which is evdent that it is rather stable for runs from C to E, while it shows an anomalous behavior for runs B and F, which deserves a deep investigation.

\begin{table}[h]\footnotesize
	\centering
	\begin{tabular}{|c||c|c|c|}
	\hline
	Run 		& 	$D_{s}$ yields per $fb^{-1}$ in data	&	SB yields per $fb^{-1}$	in data	&	SB/$D_{s}$ yields per $fb^{-1}$ in data\\ \hline 
	2017 B	&	1022		&	1368		&	0.279\\
	2017 C	&	2643	&	8838	&	0.347\\
	2017 D	&	2724		&	3841		&	0.333\\
	2017 E	&	2585	&	8422	&	0.350\\
	2017 F	&	1779		&	7140		&	0.400\\ \hline
	Whole 2017	&	2183 &	29609	&	0.357\\
	\hline
	\end{tabular}
	\caption{$D_{s} \rightarrow \phi(\mu\mu)\pi$ yields for each of the 2017 runs and yields from the 3$\mu$ sidebands in data collected with L1 DoubleMu seed. The ratio of these two yields is also reported.}
	\label{tab:Yields_Ratio}
\end{table}

\begin{figure}[h!]
\begin{minipage}{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{DsYield.pdf}
\end{minipage}
\begin{minipage}{7.5cm}
	\centering
	\includegraphics[width=7.5cm]{YieldRatio.pdf}
\end{minipage}
	 \caption{For each of 2017 runs, the $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields and the $D_{s} \rightarrow 3\mu$ yields per $fb^{-1}$, for L1_DoubleMu seeds, are reported on the left, while the ratio of the $D_{s} \rightarrow \phi(\mu\mu)\pi$ yields and the $D_{s} \rightarrow 3\mu$ yields per $fb^{-1}$ is displayed on the right.}
	\label{fig:Systematics_Ds_ratio}
\end{figure}

The uncertainty associated with the fluctuation of the produced number of $D_{s}$ in data can be estimated by using the \emph{Scale-Factor Method} \cite{ref112}.  It states that ...\\
This uncertainty can be estimated with the linear fit parameters of the plot in Fig. \ref{fig:Systematics_Ds_ratio}(right), through the equation:
\begin{equation}
\sqrt{\chi^{2}/(nd\ f-1)} \cdot \Delta p_{0}/p_{0} = 6.4 \%
\end{equation}

This is taken as a measure of systematics associated with the $D_{s}$ normalization.\\

In conclusion:\\
The overall scale factor 0.78 (Tab. \ref{tab:N_Ds}) is used to correct the $D_{s}$ yield difference between data and MC, and a 6.4 \% uncertainty is assigned to this scale factor based on 3$\mu$ sideband and $D_{s}$ ratio seen above.\\ 

In addition, the uncertainties on decay branching ratios involved in the normalization procedure has to be included. They are:
\begin{itemize}
	\item 8\% for the $\mathcal{B}(D_{s} \rightarrow \phi\pi \rightarrow \mu\mu\pi)$,
	\item 4\% for the  $\mathcal{B}(D_{s} \rightarrow \tau\nu$ .
\end{itemize}
...
\begin{itemize}
	...
	The kink algorithm is applied to the muon's inner track: it takes the difference in $\phi$ of the predicted track position and the actual recHit, squares it and divides it by the error in $\phi$ of the RecHit position. The final value corresponds to the sum of these calculated values for all the RecHits.)
	\item $segmcomp$: value of the position compatibility between the inner track and muon segment, - compatibility of the track as it is reconstructed in the tracker with the muon hypothesis (in [0, 1]); it evaluates which crossed stations have matching muon segments and assigns a probability.
...
In Fig. \ref{fig:BDT_inputs} and the distributions of all the input variables, both for the signal and for the background, are displayed, while in Fig. \ref{fig:CorrelationMatrix} are shown the linear correlation matrices both for the signal and for the background.

\newpage
\begin{figure}[h!]
 \begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=6.4cm]{p3.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=6.4cm]{cLP.pdf}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=6.4cm]{tKink.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=6.4cm]{segmComp.pdf}
 \end{minipage}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=6.4cm]{fv_nC.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=6.4cm]{fv_dphi3D.pdf}
 \end{minipage}
 \end{figure}
\begin{figure}[h!]
 \begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=6.4cm]{fv_d3Dsig.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=6.4cm]{d0sig.pdf}
 \end{minipage}
 \end{figure}
\begin{figure}[h!]
  \centering
   \includegraphics[width=6.4cm]{mindca_iso.pdf}
 \caption{Signal (blue) and background (red) distributions of the input variables used.}
...
\begin{minipage}[b]{7.3cm}
   \centering
   \includegraphics[width=7.2cm]{CorrelationMatrix_SGN_A.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.3cm}
  \centering
   \includegraphics[width=7.2cm]{CorrelationMatrix_BKG_A.pdf}
 \end{minipage}
...
Da Nadia:\\
As a general rule, a good way to know if an algorithm will work is to try it out on new datasets. The solution I have adopted is to split the data into two sets: the training set, to train the classifier, and the  test set, to test the trained classifier. By evaluating the model onn these sets it is possible to obtain an estimation of the error of the generalization of the model and of the goodness of the trained discriminator. 
The shape of the two trained discriminators is swon in Fig. \ref{fig:BDT_KStest}, for signal and bakcground muons. Test and training are superimposed.

Fig. \ref{fig:BDT_KStest} shows BDT output in different trimuon mass bins. No sign of BDT output correlation with trimuon mass is seen for either signal or background. -> parla del KS test! Kolomogorov-Smirnov (KS) test. 
This test should in theory be used only for unbinned data and not for binned one, as in the case of histograms, for which the test returns a non-uniform distribution between 0 and 1. However ROOT provides the function to implemet this statistical control: the values of the probability for binned data will be shifted slihtly higher than expected, depending on the effects of the binning.\\

If by chance the distributions of two samples under comparison are the same, then the value becomes 1.

For the evaluation of the value of the KS test I have used the procedure implemented in ROOT, not the one in TMVA because it is more precise and stabile. The resulted value, for category A, are:\\
p_KS_Signal : 0.379225
p_KS_Bkg : 0.0599964

which are a proof of the fact the the BDT is not overtrained, as it can be noticed in Fig. \ref{fig:BDT_KStest}, where the BDT response for signal and background in the test sample (filled histograms) is compared to the one for the training sample (dots).


\subsubsection*{BDT response}
\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm]{BDT_KStest.png}
	\caption{BDT_KStest.}
	\label{fig:BDT_KStest}
\end{figure}

Da Nadia:\\
One can intuitively understand overtraining from the fact that information from all past experience can be divided into two groups: information that is relevant for the future (or the prediction) and irrelevant information ("noise").
Possible solutions are: to simplify the model by selecting one with fewer parameters; to gather more training data; to reduce the noise in the training data (e.g. fix data errors and remove outliers).

A good "indicator" for the goodness of the classifier is give by the rejection of the background as a function of the efficiency of the signal, which is shown in Fig. \ref{fig:ROC_curve}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=14cm]{ROC_curve.pdf}
	\caption{Curve showing the rejection of the background as a function of the efficiency of the signal for the BDT trained as explained above.}
	\label{fig:ROC_curve}
\end{figure}

Boosted decision trees are classifiers with great characteristics, that make them particularly good for different applications. \\
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=10.6cm]{BDT_out_A.pdf}
\end{figure}
\begin{figure}[h!]  
	\centering
   \includegraphics[width=10.6cm]{BDT_out_B.pdf}
\end{figure}
\begin{figure}[h!]  
	\centering
   \includegraphics[width=10.6cm]{BDT_out_C.pdf}
    \caption{BDT response for the three categories of events. The 2 vertical bars indicate the cuts that define sub-categories.}
\label{fig:BDT_out}
 \end{figure}
...
\begin{figure}[h!]
   \centering
   \includegraphics[width=10.6cm]{BDT_2Dmap_catA.pdf}
\end{figure}
\begin{figure}[h!]  
   \centering
   \includegraphics[width=10.6cm]{BDT_2Dmap_catB.pdf}
\end{figure}
\begin{figure}[h!]  
   \centering
   \includegraphics[width=10.6cm]{BDT_2Dmap_catC.pdf}
    \caption{BDT_out_2D combined efficiencies.}
\label{fig:BDT_out_2D}
 \end{figure}
...
\begin{figure}[h!]
 \begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=7.5cm]{FinalMass_A1.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{FinalMass_A2.pdf}
 \end{minipage}
%\end{figure}
%\begin{figure}[h!]
\begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=7.5cm]{FinalMass_B1.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{FinalMass_B2.pdf}
 \end{minipage}
%\end{figure}
%\begin{figure}[h!]
\begin{minipage}[b]{7.5cm}
   \centering
   \includegraphics[width=7.5cm]{FinalMass_C1.pdf}
 \end{minipage}
 \begin{minipage}[b]{7.5cm}
  \centering
   \includegraphics[width=7.5cm]{FinalMass_C2.pdf}
 \end{minipage}
...
This contribution cannot be directly normalized by $D_{s} \rightarrow \phi(\mu\mu)\pi$.
%, but it is possible to check, indirectly, if the ratio of $B$ meson and $D$ meson production rate in data is consistent with the MC prediction. \\
...
In Fig. \ref{fig:B-D_normalization} the different behavior of the two in terms of proper decay length is shown. The red histogram is the $B \rightarrow D_{s}$ component, and the blue histogram is the prompt $D_{s}$. They are both from $D_{s} \rightarrow \phi(\mu\mu)\pi$ MC.
...
The MC predicted $f$ is 0.23. This value is used, and an uncertainty of 11\%, is taken as a systematic uncertainty\footnote{This value is taken from 2016 analysis.}. 
%\\On the other hand, the uncertainties on decay branching ratios involved in this data-driven validation procedue are included,namely, $\mathcal{B}(D_{s} \rightarrow \phi \pi \rightarrow \mu\mu\pi$)(8\%), $\mathcal{B}(B \rightarrow \tau+X)$(11\%) and $\mathcal{B}(B\rightarrow D+X)$(16\%).

The small contribution from $D^{+} \rightarrow \tau (B_{s} \rightarrow \tau)$ is added by scaling up the $D_{s} \rightarrow \tau$ ($B^{0}$ and $B^{+} \rightarrow \tau$), according to MC prediction (Tab. \ref{tab:relativeTauYield}). 100\% uncertainties are assumed for these scalings.


\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm]{B-D.pdf}
	 \caption{Prompt and non-prompt Ds contributions. The histograms are from $D_{s} \rightarrow \phi(\mu\mu)\pi$ MC. The red histogram is the $B \rightarrow D_{s}$ component, while the blue histogram stacked on the other is the prompt $D_{s}$ component.}
...
The signal prediction is first taken from MC, and then the overall scale factor 0.78 is applied. Depending on the mother particle of the $\tau$ ($D$ source or $B$ source), further corrections are applied and systematics are assigned, based on Eq. \ref{eq:N_sigD_2} and Eq. \ref{eq:N_sigB_2} respectively.\\

- Uncertainty on $D_{s}$ normalization is 6.4 \%. They apply to all events, either $D$ source or $B$ source, either triggerred by DoubleMu L1 seed, or TripleMu L1 seeds.
...
Table \ref{tab:Uncertainties} summarizes all sources of systematic uncertainties associated with modeling the signal event yields.
...
\begin{table}[h!]\footnotesize
	...
	Uncertainty on $D_{s}$ normalization [ 6.4\% ]	&	6.4\% \\
	Uncertainty on measuring f (B/D ratio) [ 11\% ]	&	3\%	\\
%	Uncertainty on n. of events triggered by trimuon trigger [ 12\% ]	&	000	\\
	Relative uncertainty in $\mathcal{B}(D_{s} \rightarrow\phi\pi\rightarrow\mu\mu\pi$) [ 8\% ]	&	8\%	\\
	Relative uncertainty in $\mathcal{B}(D_{s}\rightarrow\mu\nu)$ [ 4\% ]	&	3\%	\\
	Uncertainty on scaling $D_{s}$ to include $D^{+}$ [ 4\% ]	&	 3\% \\
	Uncertainty on scaling $B^{0}$ and $B^{+}$ to include $B_{s}$ [ 12\% ] &	4\%	\\
	Relative uncertainty in $\mathcal{B}(B\rightarrow D_{s} + ...)$ [ 16\% ]	&	4\%	\\
	Relative uncertainty in $\mathcal{B}(B \rightarrow \tau + ...)$ [ 11\% ]		&	3\%	\\
...
%	Muon momentum resolution uncertainty [ 10\% ]	&	-\\
	\hline
	\end{tabular}
	\caption{Sources of systematic uncertainties affecting signal modeling and their impact on the expected signal event yield.}
...
Dalla nota:\\
To perform statistical analysis, the CMS \emph{Higgs CombinedLimit} tool is used. Upper limits on branching fraction $\mathcal{B}(\tau \rightarrow 3\mu)$ are set using the modified frequentist CLs criterion \cite{ref118, ref119}. The chosen test statistic $q$, used to determine how signal- or background-like the data are, is based on the profile likelihood ratio. Systematic uncertainties are incorporated in the analysis via nuisance parameters and are treated according to the frequentist paradigm. The profile likelihood ratio is defined in eq. \ref{eq:q}.
...
where: 
\begin{itemize}
	\item $s$ stands for the signal that would be expected for $\mathcal{B}(\tau \rightarrow 3\mu)$ = 1,
	\item $\mu$ is a signal strength modifier, i.e. the parameter of interest in the search; b stands for backgrounds,
	\item $\theta$ are nuisance parameters describing systematic uncertainties ($\hat{\theta}_{\mu}$ maximizes the likelihood in the numerator for a given $\mu$, while $\hat{\mu}$ and $\hat{\theta}$ define the point at which the likelihood reaches its global maximum). 
\end{itemize}
 The log-normal probability density function is assumed for the nuisance parameters affecting the signal yields.
 
An unbinned version of likelihood is used, which can be written as in eq. \ref{eq:LK_unbin}.
...
where: 
\begin{itemize}
	\item $S$ is the total number of signal events,
	\item $B$ is the total number of background events,
	\item the index $i$ runs over all events,
	\item $\mathcal{P}(x_{i}| \mu\ s + b)$ is an event density function of $x$ such that the expected event rate in vicinity of a given value of $x$ is predicted as $\mathcal{P}(x|\mu\ s+b)dx$.
\end{itemize}

The ratio of probabilities to observe a value of the test statistic at least as large as the one observed in data, $q^{obs}_{\mu}$, under the signal+background ($s+b$) and background-only ($b$) hypotheses,

\begin{equation}
\mathrm{CL_{s}}\ =\ \frac{P\ (q_{\mu} \geq q_{\mu}^{obs}\ |\mu\cdot\ s+b)}{P\ (q_{\mu}\geq q_{\mu}^{obs}\ |\ b)} \leq 0.1
\end{equation}
is used as the criterion for excluding the signal at the 90\% confidence level.

The observed combined upper limit at 90\% CL is $8.9 \times 10^{-8}$, while the expected limit is $9.9 \times 10^{-8}$. (At 95\% CL the observed upper limit is $1.1 \times 10^{-7}$, while the expected limit is $1.2 \times 10^{-7}$). 

The plot shown in Fig. \ref{fig:Final_plot} combines all six mass distributions, with all events and predictions from each contributing distribution i scaled by factors proportional to $S_{i} /(S+B_{i})$ ratios assessed using the expected signal ($S$) and background ($B$) event rates at the trimuon mass equal to the tau lepton mass. The obtained distribution is scaled so that the total “signal event yield” in the plot equals to the sum of the actual expected signal yields in all six categories. The plot gives an overall qualitative representation of the analysis’s power to observe a signal over background.
